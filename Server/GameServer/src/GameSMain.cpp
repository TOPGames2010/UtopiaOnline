...
int main(int argc, char* argv[])
{
	...
	const auto ret = localtime_s(std::addressof(calendar_time), std::addressof(now));
	if (ret != 0)
		return FALSE;

	const int currentYear = calendar_time.tm_year + 1900;
	const int currentMonth = calendar_time.tm_mon + 1; // tm_mon is 0-based (0 = Jan)

	// Block execution if after July 2025
