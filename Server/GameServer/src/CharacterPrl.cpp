...
{T_B
	...
	{
		//--------------------------------------------------------------//
	case CMD_CM_SecretPacket: {
		//first packet key must match
		if (const auto keys = READ_SHORT(pk); keys == 9877)
		{
		...
		//--------------------------------------------------------------//
		// new Teleport  button mothannakh
	case CMD_CM_TelePort_NPC: {
		...
		g_CParser.DoString("TeleportButton", enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this, DOSTRING_PARAM_END);
		break;
	}
	case CMD_CM_RANK:{
		if (DWORD COOLDOWN = GetTickCount(); ShowRankColD > COOLDOWN)
		{
