...
inline int lua_IsOfflineMode(lua_State* L)
{
	if (const BOOL bValid = lua_gettop(L) == 1 && lua_islightuserdata(L, 1); !bValid) {
		E_LUAPARAM
		return 0;
	}

	const CCharacter* pCha = static_cast<CCharacter*>(lua_touserdata(L, 1));
	if (!pCha) {
	...
	lua_pushboolean(L, pCha->IsOfflineMode());
	return 1;
}
