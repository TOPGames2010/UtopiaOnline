...
//-----------------------------------------------------------------------------
void CMPPartSys::setIsWingRender(const bool isWing) {
	isWingRender = isWing;
	for (auto i = 0; i < _iParNum; ++i) {
		if (const auto nParticle = _vecParticle[i]; !nParticle) {
			continue;
		}
		const auto npart = _bMediay ? &_CPPart[i] : _CPPart;
		if (!npart)
			continue;
		for (int j = 0; j < npart->m_iEffNum; j++) {
			if (npart->m_vecEffect[j]) {
				npart->m_vecEffect[j]->setisWingRender(isWing);
			}
		}
	}
}
//-----------------------------------------------------------------------------
void CMPPartSys::setisWeaponGlow(const bool value) {
	isWeaponGlow = value;
	for (auto i = 0; i < _iParNum; ++i) {
		if (const auto nParticle = _vecParticle[i]; !nParticle) {
			continue;
		...
		for (int j = 0; j < npart->m_iEffNum; j++) {
			if (npart->m_vecEffect[j]) {
				npart->m_vecEffect[j]->setisWeaponGlow(value);
			}
		}
	}
}
