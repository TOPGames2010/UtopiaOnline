...
void	I_Effect::SetVertexShader() const
{
	m_pDev->SetVertexShader(ResMgr.GetVShaderByID(_iVSIndex));
	m_pDev->SetVertexDeclaration(ResMgr.GetVDeclByID(_iVSIndex));
}

//_iVSIndex. @mothannakh
void I_Effect::setisWeaponGlow(const bool eType)
{
	//if its weapon glow use render 1 
	_iVSIndex = 1;//eff2.vsh
	isWeaponGlow = eType;
}
