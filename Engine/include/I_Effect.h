...
public:
	...
	//}
	void		 SetVertexShader() const;//		{ m_pDev->SetVertexShader(*_pdwVShader);}
	//!渲染
	virtual void Begin()
	{
		if (m_pCModel)
...
public:
	//! 得到设置桢数量
	WORD			getFrameCount() const { return _wFrameCount;}
	WORD			setFrameCount(WORD  wFrameCount){ return _wFrameCount = wFrameCount;}

	//!得到类型
	EFFECT_TYPE		getType() const { return _eEffectType;}
	void			setType(EFFECT_TYPE eType)		{ _eEffectType =  eType;}

	//!得到长度
	float			getLength() const { return _fLength;}
	void			setLength(float fLength)		{ _fLength = fLength;}

	//!得到帧时间
	float			getFrameTime(WORD wIndex) const { return _vecFrameTime[wIndex];}
	void			setFrameTime(WORD wIndex,float fTime){ _vecFrameTime[wIndex] = fTime;}

	//!得到帧大小
	D3DXVECTOR3		getFrameSize(WORD wIndex) const { return _vecFrameSize[wIndex];}
	...
	//!得到帧坐标时间
	float	getFrameCoordTime() const { return m_CTexCoordlist.m_fFrameTime; }
	void    setFrameCoordTime(float fTime)			{ m_CTexCoordlist.m_fFrameTime = fTime;}

	int		getFrameCoordCount() const { return (int)m_CTexCoordlist.m_vecCoordList.size(); }
	...
	//set wing render type isWingRender
	bool getisWingRender() const { return isWingRender; }
	void setisWingRender(const bool eType) { isWingRender = eType; }
	//isWeaponGlow
	bool getisWeaponGlow() const { return isWeaponGlow; }
	void setisWeaponGlow(bool eType);// { isWeaponGlow = eType; }

	//!得到帧坐标
	void	getFrameCoord(TEXCOORD& vecOutCoord, WORD wIndex) const
	{
		vecOutCoord.clear();
	...
	///////////////////////////////////////////////////////////
	bool	 IsModelRect() const { return m_strModelName == MESH_RECT;}
	bool	 IsModelPlaneRect() const { return m_strModelName == MESH_PLANERECT;}
	bool	 IsModelTri() const { return m_strModelName == MESH_TRI;}
	...
	//////////////////////////////////////////////////////////////////////////
	//!得到插值大小
	void	GetLerpSize(D3DXVECTOR3 *pSOut, WORD wIdx1, WORD wIdx2, float fLerp) const
	{
	...
	//!得到插值角度
	void	GetLerpAngle(D3DXVECTOR3 *pSOut, WORD wIdx1, WORD wIdx2, float fLerp) const
	{
	...
	//!得到插值位置
	void	GetLerpPos(D3DXVECTOR3 *pSOut, WORD wIdx1, WORD wIdx2, float fLerp) const
	{
	...
	//!得到插值颜色
	void	GetLerpColor(D3DXCOLOR *pSOut, WORD wIdx1, WORD wIdx2, float fLerp) const
	{
	...
	D3DXMATRIX*	 getBillBoardMatrix() const { return _SpmatBBoard;}

	bool		 IsBillBoard() const { return _bBillBoard;}

	void		 SetLoop(bool	bloop)						{ _bRotaLoop = bloop; }
	bool		 IsRotaLoop() const { return _bRotaLoop;}
	D3DXVECTOR4* GetRotaLoop()								{ return &_vRotaLoop;}

	bool		 IsAlpah() const { return _bAlpha;}
	void		 EnableAlpha(bool balpha)					{ _bAlpha = balpha;}


	D3DBLEND	GetAlphaTypeSrc() const { return _eSrcBlend;}
	D3DBLEND	GetAlphaTypeDest() const { return _eDestBlend;}
	...
	void	GetTobParam(int nFrame, int&	nSegments,float&	rHeight,float& rRadius,float&	rBotRadius) const
	{
	...
	int		IsUseParam() const { return _iUseParam;}

	bool	IsRotaBoard() const { return _bRotaBoard;}
...
protected:
	...
	bool isWingRender{ false }; //here we define wings on it own render 
	bool isWeaponGlow{ false };//new function we use to know f its weapon glow or not
	//！动画的时间长度(秒)
	float				_fLength; 
	//！动画桢数量
	WORD				_wFrameCount;
