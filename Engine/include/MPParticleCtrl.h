...
public:
	...
	void setIsWingRender(const bool isWing) {
		isWingRender = isWing;
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n]) {
				m_vecPartSys[n]->setIsWingRender(isWing);
			}
		}
	}
	//setisWeaponGlow
	void setisWeaponGlow(const bool value) {
		isWeaponGlow = value;
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n]) {
				m_vecPartSys[n]->setisWeaponGlow(value);
			}
		}
	}
	void setDir(const D3DXVECTOR3* pvPos)
	{
		for (int n = 0; n < m_iPartNum; ++n)
		{
...
public:
	...
	bool isWingRender{ false };//i add these for sake to track it from anywhere
