...
private:
	virtual void	_Run()=0;					//锟斤拷锟斤拷锟斤拷override锟斤拷锟斤拷应
...
private:
	RunCtrlThrd(RunCtrlMgr<T> * mgr,short thrdpos):m_mgr(mgr),m_thrdpos(thrdpos),m_timer(false)//使锟斤拷QueuePerformanceX锟斤拷时
	{
	}
	virtual	~RunCtrlThrd() = default;
	Task	*	Lastly()			//锟斤拷锟斤拷锟斤拷mgr删锟斤拷
	{
		return 0;
	}
	long Process()
	{
		...
		while(!GetExitFlag()){try{
			...
			AddUsage(l_tick);
			if(T::TimePrecision>long(l_tick))::Sleep(T::TimePrecision-long(l_tick));	//锟斤拷锟斤拷锟桔伙拷CPU锟斤拷锟侥碉拷时锟斤拷锟姐法
		}catch(...){}}
		return 0;
	}
public:
