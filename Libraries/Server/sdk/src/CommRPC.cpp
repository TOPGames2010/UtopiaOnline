...
long OnServeCall::Process()
{
	try
	{
		...
		m_sessid	+=SESSFLAG;									//锟斤拷锟较凤拷锟截憋拷志
...
RPCMGR::RPCMGR(TcpCommApp *tca):__tca(tca){
	if(!__tca)
	{
		THROW_EXCP(excp,"锟斤拷锟斤拷锟结供通锟斤拷应锟矫讹拷锟斤拷");
	}
}
//==============================================================================
...
long	RPCMGR::AsynCall(DataSocket *datasock,WPacket in_parameter,void *pointer)
{
	...
	if(l_rpcinfo)
	{
		try
		{
			...
			if( l_fncall )
			{
				l_fncall->m_isSync	=false;			//锟斤拷志锟届步锟斤拷锟斤拷
	...
	if(l_retval)								//senddata锟斤拷锟斤拷锟斤拷锟斤拷
	{
...
RPacket RPCMGR::SyncCall(DataSocket *datasock,WPacket	in_parameter,uLong ulMilliseconds)
{
	...
	if(l_rpcinfo)
	{
		try
		{
			...
			if( l_fncall )
			{
				l_fncall->m_isSync	=true;			//锟斤拷志同锟斤拷锟斤拷锟斤拷
