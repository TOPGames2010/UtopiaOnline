	...
	if(ulMilliseconds>l_tick)
	{
		...
		if((l_sem ==WAIT_OBJECT_0) || (l_item->m_semWait.trylock() ==WAIT_OBJECT_0))//(l_item->m_iscall ==2)
		{
			if(l_item->m_iscall !=2)
			{
				...
				l_line<<newln<<"PKQueue::SyncPK:锟斤拷志锟斤拷锟斤拷[volatile char	m_iscall]锟竭筹拷之锟戒发锟斤拷锟斤拷同锟斤拷锟斤拷锟斤拷";
			}
			m_mut.unlock();
			...
			{
				LogLine l_line(g_dbclog);
				l_line<<newln<<"PKQueue::SyncPK 锟斤拷锟截帮拷没锟斤拷锟斤拷";
			}
			l_item->Free();
			return l_retpk;
		}else
		{
			...
			m_mut.unlock();
			LogLine l_line(g_dbclog);
			l_line <<newln<<"PKQueue::SyncPK 锟斤拷时:"<<ulMilliseconds<<"ms";
			return 0;
		}
	}else
	{
		LogLine l_line(g_dbclog);
		l_line <<newln<<"PKQueue::SyncPK 锟斤拷时(锟斤拷锟斤拷锟斤拷锟窖撅拷锟斤拷时):"<<l_tick<<"ms";
		return 0;
	}
}
		...
		case 1:
			{
				...
				{
					LogLine l_line(g_dbclog);
					l_line<<newln<<"PKQueue::PeekPacket 锟斤拷锟斤拷锟届常";
					l_line<<endln;
					auto const l = std::lock_guard{m_mut};
