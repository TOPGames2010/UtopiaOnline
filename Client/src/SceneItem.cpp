...
mParentCharacter(nullptr), mEffect( nullptr )
{
...
{
    //we ignore create effect in select cha scene 
    if (const auto pScene = dynamic_cast<CSelectChaScene*>(CGameApp::GetCurScene()); pScene) {
        //LG("error", "select cha scene ignored %s\n", pProperty->m_strName.c_str());
        return;
    }
	...
	for( int i=0; i<pInfo->GetEffectNum(nCharID); i++ )
	{
		...
		const float fEffectScale = Forge.pRefineInfo->fChaEffectScale[nCharID];
        ...
        pEffect->SetisWeaponGlow(true);//mark it as weapon glow 
