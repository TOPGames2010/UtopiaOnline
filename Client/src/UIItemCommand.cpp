...
bool CItemCommand::UseCommand(const bool isRightClick)
{
	...
	if (!pCha)
		return false;
	const bool isSwitchItem = _pItem->sType == 77 || _pItem->sType == 90 || _pItem->sType == enumItemTypePet
		|| _pItem->sType == 81 || _pItem->sType == 82 || _pItem->sType == 83 || _pItem->sType == 88 || _pItem->sType < 31;
	//this allow switch item while in stun states 
	if (!isSwitchItem && pCha->GetChaState()->IsFalse(enumChaStateUseItem)) {
		CGameApp::SysInfo(g_oLangRec.GetString(678));
		return false;
	}

	if (const auto pOwn = dynamic_cast<CGoodsGrid*>(GetParent()); pOwn)
	{
		CCharacter* p_cha = g_stUIBoat.FindCha(pOwn);
		if (!p_cha) {
			return false;
		}
		...
		if (_pItem->sType == 66 || (864 <= _pItem->lID && _pItem->lID <= 866))
		{
			CActor* pActor = CGameScene::GetMainCha()->GetActor();

			const auto pState = new CFeteState(pActor);
			pState->SetFeteGridID(GridID);
			pActor->SwitchState(pState);
			return false;
		}
		if (GetItemInfo()->sType == 71) // 
		{
			CCharacter* cha = CGameScene::GetMainCha();
			if (!cha) return false;

			const int nSkillID = Str2Int(GetItemInfo()->szAttrEffect);
			...
			// �жϼ����ܷ��ں���ʩ��
			if (cha->IsBoat() && pSkill->chSrcType != 2)
			{
				CGameApp::SysInfo(g_oLangRec.GetString(879));
				return false;
			}

			// �жϼ����ܷ���½����ʩ��
			if (!cha->IsBoat() && pSkill->chSrcType != 1)
			{
				CGameApp::SysInfo(g_oLangRec.GetString(880));
				return false;
			}

			if (const int nCoolDownTime = Str2Int(pSkill->szFireSpeed); nCoolDownTime > 0)// 
			{
				const DWORD nCurTickCount = CGameApp::GetCurTick() - 500;	// 

				if (const auto it = _mapCoolDown.find(nSkillID); it != _mapCoolDown.end() && it->second + nCoolDownTime >= nCurTickCount)
				{
					// cooldown ��
					CGameApp::SysInfo(g_oLangRec.GetString(898), (it->second + nCoolDownTime - nCurTickCount) / 1000 + 1);
					return false;
				}

				_mapCoolDown[nSkillID] = CGameApp::GetCurTick();
			}

			// �����������ͷŶ����Χ
			if (pSkill->GetDistance() <= 0)
			{
				const auto attack = new CAttackState(cha->GetActor());
				attack->SetSkill(pSkill);
				attack->SetTarget(cha);
				attack->SetCommand(this);
				return cha->GetActor()->SwitchState(attack);
			}

			//pCha->GetActor()->Stop();
			cha->ChangeReadySkill(nSkillID);
			return false;
		}
		...
		//|| _ItemData.sID == 5939
			...
			//atm we keep 500ms for skill bar using items and switch items for better control 
		if (isSwitchItem && p_cha == CGameScene::GetMainCha() && g_stUIBoat.GetHuman() == p_cha) {
			//this for lock stone items we don't use it anymore
			...
			if (static DWORD time = 0; CGameApp::GetCurTick() > time) {
				time = CGameApp::GetCurTick() + 500; // static_cast<DWORD>(GetMillisecondsFromSeconds(1)) second cooldown
				CS_BeginAction(p_cha, enumACTION_ITEM_USE, &param);
			}
			else {
				DWORD remainingTime = (time - CGameApp::GetCurTick()) / 1000; // Convert milliseconds to seconds
				const std::string msgCd = std::format("[{}] is cooling down. Remaining time: {} seconds.", _pItem->szName, remainingTime);
				CGameApp::SysInfo(msgCd.c_str());
			}

			//old function force to stop 
			//CActor* pActor = g_stUIBoat.GetHuman()->GetActor();
			//const auto pState = new CEquipState(pActor);
			//pState->SetUseItemData(param);
			//pActor->SwitchState(pState);
			return true;
		}
		//here other items been used like food etc 
		if (!IsAllowUse())
			return false;
		if (!StartCommand())
			return false;
	...
	if (const auto pOne = dynamic_cast<COneCommand*>(GetParent())) {
