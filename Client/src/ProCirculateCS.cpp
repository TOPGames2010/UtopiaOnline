...
void CProCirculateCS::EndAction( CActionState* pState )
{
	...
	// log
	if (CCharacter *pMainCha = CGameApp::GetCurScene()->GetMainCha(); pMainCha)
	{
...
bool CProCirculate::Connect(const char *hostname,unsigned short port,unsigned long timeout) const
{
	return pCNetIf->m_connect.Connect(hostname,port,timeout);
}
void CProCirculate::Disconnect(int reason) const
{
...
void CProCirculate::OpenTeleportNPC() const
// mothannakh// Teleport Npc
{
	WPacket pk = pCNetIf->GetWPacket();
	pk.WriteCmd(CMD_CM_TelePort_NPC);
	pCNetIf->SendPacketMessage(pk);
}
