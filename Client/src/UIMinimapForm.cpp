	...
	if (auto checkFollow = dynamic_cast<CCheckBox*>(frmBigmap->Find("checkFollow")); checkFollow)
	{
		checkFollow->evtCheckChange = [](CGuiData* pSender) {
			if (auto scene = CGameApp::GetCurScene(); scene)
			{
				if (auto map = scene->GetLargerMap(); map)
				{
					map->SetFollow(dynamic_cast<CCheckBox*>(pSender)->GetIsChecked());
				}
			}
		};
	}
...
{
	if (const CWorldScene* pWorldScene = dynamic_cast<CWorldScene*>(CGameApp::GetCurScene()); !pWorldScene) return;
...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	else if (name == "btnVIP")
	{
		if (const auto pCha = CGameScene::GetMainCha(); pCha) {
			if (!pCha->GetPlayerVip()) {
				g_pGameApp->ShowMidText("This Feature Available for vip/nitro players! buy your package first!");
				return;
			}
			if (const auto proCir = dynamic_cast<CProCirculateCS*>(g_NetIF->GetProCir()); proCir) {
				proCir->OpenTeleportNPC();
			}
		}
	}
	...
	{
		const auto proCir = dynamic_cast<CProCirculateCS*>(g_NetIF->GetProCir());
		proCir->OpenRankings();
	}
