...
void NetLoginFailure(unsigned short Errno)
{
	...
	case ERR_AP_INVALIDPWD:
		{
			...
			if( dwCount >= 3 )
			{
				...
				{
					CGameApp::MsgBox( g_oLangRec.GetString(251) );
	...
	case ERR_AP_BANUSER:
		{
			CGameApp::MsgBox( g_oLangRec.GetString(252) );
			return;
		}
	case ERR_AP_INVALIDUSER:
		{
			CGameApp::MsgBox( g_oLangRec.GetString(253) );
			return;
		}
...
void	NetDelCha(unsigned short Errno)		//��NetRetCode.h�ļ��в�ѯerrno����
{
	...
	{
		CGameApp::Waiting(false);
		CGameApp::MsgBox(g_oLangRec.GetString(802));
		return;
	}
	case ERR_PT_MULTICHA:
		CGameApp::Waiting(false);
		CGameApp::MsgBox("You cannot delete a character still playing");
		return;
	default:
	{
...
void NetCreatePassword2(unsigned short Errno)
{
	...
	if( Errno == ERR_SUCCESS )
	{
		...
		CSelectChaScene* pSelChaScene = dynamic_cast<CSelectChaScene*>(CGameApp::GetCurScene());
		if(pSelChaScene)
		{
	...
	{
		// ϵͳæ
		CGameApp::MsgBox(g_oLangRec.GetString(172), "");
	}
	else if( Errno == ERR_PT_INVALID_PW2 )
	{
		// �����������������һ�����Ѵ���
		CGameApp::MsgBox(g_oLangRec.GetString(801));
	}
	else
	{
		// δ֪����
		CGameApp::MsgBox(g_oLangRec.GetString(375));
	}
}
...
void NetSwitchMap(stNetSwitchMap &switchmap)
{
	...
	if( switchmap.sEnterRet!=ERR_SUCCESS )
	{
		...
		{
			CGameApp::MsgBox( g_oLangRec.GetString(271) );
			CGameApp::Waiting( false );
		}
		else
		{
...
void NetChangeChaPart( CCharacter* pCha, stNetLookInfo &SLookInfo )
{
	...
	else
	{
		...
		if( pCha->GetMainType()==enumMainPlayer )
		{
			...
			if( SLookInfo.chSynType==enumSYN_LOOK_SWITCH )
			{
				GUI::CStartMgr::RefreshMainFace( SPart );
...
void NetMapMask(unsigned int nID, BYTE *pMask, long lLen)
{
	...
	else
	{
		...
		CGameApp::SysInfo( g_oLangRec.GetString(277) );
	}
...
{
	xShipFactory *pShip = 
		((CWorldScene*)CGameApp::GetCurScene())->GetShipMgr()->_factory;
	if (!pShip)
		return;
...
{
    ((CWorldScene*)CGameApp::GetCurScene())->GetShipMgr()->_factory->Update((xShipBuildInfo*)&Info, 1, 0);
}
...
{
	((CWorldScene*)CGameApp::GetCurScene())->GetShipMgr()->
...
void NetChangeKitbag(DWORD dwChaID, stNetKitbag& SKitbag)
{
	...
	int fastItemCount = 0;
	short fastItemSlot = -1;
	std::array<int, 36> fastCommandSlot{ -1 };
	fastCommandSlot.fill(-1); //somehow if we do not use fill -1 logic go wrong for index c.c so keep it
	//std::fill_n(fastCommandSlot, 36, -1);
    ...
    for( int i=0; i<count; i++ )
    {
		...
		if (pGrid[i].SGridContent.sID > 0) {
			CItemRecord* item = GetItemRecordInfo(pGrid[i].SGridContent.sID);
            if( !item )
            {
			...
			auto pObj = dynamic_cast<CItemCommand*>(grd->GetItem(pGrid[i].sGridID));
			if (chType == enumSYN_KITBAG_EQUIP) {
				if (fastItemCount == 0) {
					fastItemCount = 1;
					fastItemSlot = pGrid[i].sGridID;
					if (pObj) {
			...
			if( SKitbag.chBagType!=1 && nMarginNum > 0 )	// modify by Philip.Wu  2006-06-21  ʰ��0����BUG����
			{
				switch( chType )
				{
				case enumSYN_KITBAG_PICK: CGameApp::SysInfo(g_oLangRec.GetString(283), item->szName, nMarginNum );		break;
				//case enumSYN_KITBAG_FROM_NPC:	g_pGameApp->SysInfo("NPC��[%s �� %d]!", item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_SYSTEM: CGameApp::SysInfo(g_oLangRec.GetString(284), item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_TRADE: CGameApp::SysInfo(g_oLangRec.GetString(285), item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_FORGES: CGameApp::SysInfo(g_oLangRec.GetString(286), item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_FORGEF: CGameApp::SysInfo(g_oLangRec.GetString(287), item->szName, nMarginNum );	break;
				}
			}
        }
        else
	...
	case enumSYN_KITBAG_EQUIP: {
		...
		if (fastItemSlot != -1) {
			...
			for (const int i : fastCommandSlot) {
				if (i == -1) {
					break;
				}
				// CGameApp::SysInfo("fastslot :%d , i :%d", fastItemSlot,i);
				if (const auto pObj1 = dynamic_cast<CItemCommand*>(grd->GetItem(fastItemSlot)); pObj1) {
					g_stUIEquip.FastChange(i, fastItemSlot, defItemShortCutType, true);
				}
			}
		}
		break;
