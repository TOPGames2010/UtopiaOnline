...
// ��ʼ�˵�
class CStartMgr final : public CUIInterface
{
public:
	CForm* frmTargetInfo;
	...
	CList*			GetNpcList() const {return lstNpcList;}
	CCheckBox*		chkID;
	...
	static void			RefreshMainFace(const stNetChangeChaPart& stPart );

	void			RefreshPet( CItemCommand* pItem );
	void			RefreshPet( SItemGrid pItem,SItemGrid pApp );
	void			RefreshPet() const;

	void			SetIsLeader( bool v ) const;
	...
	void			UpdateShipSailForm() const;

	void			SetIsNewer( bool v )	{ _IsNewer = v;			}

	void			SysLabel( const char *pszFormat, ... ) const;
	void			SysHide() const;
