...
public:
	...
	void SetIsWingEffect(const bool v) {
		isWingEffect = v;
		if (_pEffCtrl) {
			_pEffCtrl->GetPartCtrl()->setIsWingRender(isWingEffect);
		}
	}
	void SetisWeaponGlow(const bool v) {
		isWeaponGlow = v;
		if (_pEffCtrl) {
			_pEffCtrl->GetPartCtrl()->setisWeaponGlow(v);
		}
	}
	[[nodiscard]] bool GetisWeaponGlow() const { return isWeaponGlow; }
	[[nodiscard]] bool GetisWingEffect() const { return isWingEffect; }
