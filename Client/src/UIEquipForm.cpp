...
CSkillRecord* CEquipMgr::FindSkill(const int nID) const
{
...
//defItemShortCutType
void CEquipMgr::FastChange(const int nIndex, const short sGridID, const char chType, const bool update) {

	if (update) {
		_pFastCommands[nIndex]->AddCommand(GetGoodsGrid()->GetItem(sGridID));
	}
	stNetShortCutChange param = {};
	param.chIndex = static_cast<char>(nIndex);
	param.chType = chType;
	param.shyGrid = sGridID;

	CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_SHORTCUT, &param);
	auto& [ch_type, byGridID] = _stShortCut;
	ch_type[nIndex] = chType;
	byGridID[nIndex] = sGridID;
}

void CEquipMgr::_evtFastChange(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	if (const auto pGrid = dynamic_cast<CGoodsGrid*>(CDrag::GetParent()); pGrid && pGrid != g_stUIEquip.GetGoodsGrid())
		return;

	const auto pFast = dynamic_cast<CFastCommand*>(pSender);
	if (!pFast)
		return;
	...
	if (!_GetCommandShortCutType(pItem, chType, sGridID)) {
		isAccept = false;
		CGameApp::SysInfo(g_oLangRec.GetString(553), pItem->GetName());
		return;
	}
	g_stUIEquip.FastChange(nIndex, sGridID, chType);

	//	CGameApp::SysInfo("it called index:%d grid: %d type :%d", nIndex, sGridID, chType);
}
...
bool CEquipMgr::_GetCommandShortCutType( CCommandObj* pItem, char& chType, short& sGridID )
{
    ...
    {
	    const auto pSkill = dynamic_cast<CSkillCommand*>(pItem);
    ...
    {
	    if(const auto pSkill = dynamic_cast<CSkillCommand*>(pItem) ; pSkill)
        {
...
void CEquipMgr::UpdateShortCut(const stNetShortCut& stShortCut) {
...
void CEquipMgr::DelFastCommand(CCommandObj* pObj) {
	if (pObj && pObj->GetIsFast()) {
		if (const CFastCommand* pFast = CFastCommand::FintFastCommand(pObj); pFast) {
			const int nIndex = pFast->nTag;
			stNetShortCutChange param = {};
			param.chIndex = nIndex;
			CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_SHORTCUT, &param);
			if (pFast->GetCommand()) {
				if (const auto iconPath = pFast->GetCommand()->GetIconPath(); !iconPath.empty()) {
					SetTextureAsSkillIcon(iconPath, false);
				}
			}

			auto& [chType, byGridID] = _stShortCut;
			chType[nIndex] = 0;
			byGridID[nIndex] = 0;
			pFast->DelCommand();
		}
