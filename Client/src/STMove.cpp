...
void CWaitMoveState::SetPreMoveTime(const long time) {
	_ulPreMoveTime = time;

	MaxPing = std::max<long>(MaxPing, time);
	MinPing = std::min<long>(MinPing, time);

	LastPing[LastPingCnt] = time;

	int nCnt = LastPingCnt;
	for (int& i : LastPingShow) {
		i = LastPing[nCnt];
		nCnt--;
		if (nCnt < 0)
			nCnt = LAST_NUM - 1;
	...
	nTotalPing += time;
	nTotalPingCnt++;
	fAveragePing = static_cast<float>(nTotalPing) / static_cast<float>(nTotalPingCnt);
	...
	nMaxFPS = std::max<DWORD>(nMaxFPS, g_Render.GetFPS());
	nMinFPS = std::min<DWORD>(nMinFPS, g_Render.GetFPS());
	nTotalFPS += g_Render.GetFPS();
	nTotalFPSCnt++;
	fFPSAverage = static_cast<float>(nTotalFPS) / static_cast<float>(nTotalFPSCnt);
...
void CWaitMoveState::RenderPing() {
	...
	constexpr int nY = 80;	 // Y-coordinate for text
	...
	GetRender().FillFrame(nX, nY, nX + w, nY + h);
