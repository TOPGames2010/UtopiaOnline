...
void CStartMgr::SetMonsterInfo() const
{
	...
	std::vector vect(defCHA_INIT_ITEM_NUM, vector(2, 0));
	...
	char buf[64];
	const auto MobLvlTxt = charInfo->lLv > chaLevel + 5 ? "???" : to_string(charInfo->lLv);
	LabMobLevel->SetCaption(MobLvlTxt.c_str());
...
void CStartMgr::SetTargetInfo(CCharacter* p_Target)
{
	...
	if (p_Target && !p_Target->IsPlayer()) {
		...
		if (MonstetDropID == p_Target->getMobID() || p_Target->getMobID() <=4)
			return;
		char chaLv[5];
...
void CStartMgr::UpdateBackDrop() 
{
    ...
    if (nArea) {
		_snprintf_s(buf, sizeof buf, _TRUNCATE, "texture/ui/corsairs/npcBackdrop/%d.tga", nArea);
    } else {
...
void CStartMgr::UpdateShipSailForm() const
{
	CCharacter* pMain = CGameScene::GetMainCha();
...
void CStartMgr::SysLabel( const char *pszFormat, ... ) const
{
	char szBuf[2048] = { 0 };
...
void CStartMgr::RefreshMainFace(const stNetChangeChaPart& stPart )
{
	if( pMainCha )
	{
		static stNetTeamChaPart stTeamPart;	
...
void CStartMgr::SetIsLeader( bool v ) const
{
	imgLeader->SetIsShow( v );
}

bool CStartMgr::GetIsLeader() const
{
...
void CStartMgr::RefreshPet() const
{
	if(g_stUIBoat.GetHuman()){
