...
function ConditionsTest(character, conditions, param1, param2, npc)
    ...
    if conditions[1] == nil then
        ...
        for i = 1, num, 1 do
            ...
            elseif conditions[i].func == HasFairyActive then
				PRINT("ConditionsTest:HasFairyActive, p1 = ", conditions[i].p1)
				local ret = HasFairyActive(character, conditions[i].p1)
				if ret ~= LUA_TRUE then
					PRINT("ConditionsTest:HasFairyActive = false")
					return LUA_FALSE
				end	           			
            else
...
function ActionsProc(character, actions, npc, page, misid, scriptid, param1, param2)
    ...
    for i = 1, num, 1 do
		...
		elseif actions[i].func == Give_ElfEXP then
			PRINT( "ActionProc:Give_ElfEXP, p1 = , p2 = ", actions[i].p1, actions[i].p2 )
			local ret = Give_ElfEXP(character, actions[i].p1, actions[i].p2)
			if ret ~= LUA_TRUE then
				PRINT("ActionProc:Give_ElfEXP = false")
				return LUA_FALSE
			end	   
        end
    end
...
function XmasNotice(role, value)
    ...
    message[4] = "С��ķ:����~����~�Ǹ�������ʶ,����" .. cha_name .. ",�ҳ����һ��Ҫ������ô�?!"
...
function HasFairyActive(Player,slot)
    local Fairy = GetEquipItemP(Player, slot)
    if Fairy == nil then
        SystemNotice(Player, "Please equip your fairy first, then try again.")
        return LUA_FALSE
    end         
    return LUA_TRUE
end
