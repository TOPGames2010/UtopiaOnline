...
function ItemUse_IDBOX(role, Item)
    ...
    if lv < 60 then
        ...
        SystemNotice(role,
            "Event have ended, Thank You for your participation. Please refer to Pirate King Online official website for future events!")
    end
end
...
function FairyFruit(role, Item)
    ...
    local STR, CON, AGI, ACC, SPR = GetFairyStats(Fairy)
    local ItemID = GetItemID(Item)
    local fairyAttr = Server.Fairy.Fruit[ItemID].ATTR

    local attributes = {
        [ITEMATTR_VAL_STR] = CON + AGI + ACC + SPR,
        [ITEMATTR_VAL_CON] = STR + AGI + ACC + SPR,
        [ITEMATTR_VAL_AGI] = STR + CON + ACC + SPR,
        [ITEMATTR_VAL_DEX] = STR + CON + AGI + SPR,
        [ITEMATTR_VAL_STA] = STR + CON + AGI + ACC
    }

    local isMixed = attributes[fairyAttr]
    if isMixed and isMixed > 0 then
        SystemNotice(role, "You Cannot feed the fairy with multiple fruits at once.")
        UseItemFailed(role)
        return
    end
    ...
    if GetItemType(Item) == 58 and GetItemType(Fairy) == 59 then
        ...
        if Check_Exp == 0 then
            ...
            FairyLevel(role, Fairy, fairyAttr, Server.Fairy.Fruit[ItemID].LV, 0)
        end
    end
end
...
function ItemUse_CJBOX(role, Item)
    ...
    if Item_newJLID == 231 or Item_newJLID == 232 or Item_newJLID == 233 or Item_newJLID == 234 or Item_newJLID == 235 or
        Item_newJLID == 236 or Item_newJLID == 237 or Item_newJLID == 681 then
        Part1_JL = 1
...
function ItemUse_JingLingBOX(role, Item)
    ...
    if c ~= -1 then
        ...
        if GoodItem == 1 then
            local message = "" .. GetChaDefaultName(role) .. " opens a " .. GetItemName(ItemID) .. " and obtains " ..
                                GetItemName(FairyID) .. ""
            Notice(message)
        end
    end
...
function ItemMedKit(role, Item)
    ...
    if Server.Restore.Life[ItemID].Cooldown ~= 0 then
        ...
        if Cooldown > 0 then
            BickerNotice(role, string.format("Item:[%s] in cooldown mode, remaining time is %d sec(s)",
                GetItemName(GetItemID(Item)), Cooldown))
            UseItemFailed(role)
            return
        end
    end
    if Server.Restore.Life[ItemID] ~= nil then
        ...
        if Server.Restore.Life[ItemID].Amount.HP ~= 0 or Server.Restore.Life[ItemID].Percentage.HP ~= 0 then
            local HP = GetChaAttr(role, ATTR_HP) + Server.Restore.Life[ItemID].Amount.HP +
                           (GetChaAttr(role, ATTR_MXHP) * (Server.Restore.Life[ItemID].Percentage.HP))
        ...
        if Server.Restore.Life[ItemID].Amount.SP ~= 0 or Server.Restore.Life[ItemID].Percentage.SP ~= 0 then
            local SP = GetChaAttr(role, ATTR_SP) + Server.Restore.Life[ItemID].Amount.SP +
                           (GetChaAttr(role, ATTR_MXSP) * (Server.Restore.Life[ItemID].Percentage.SP))
        ...
        if Server.Restore.Life[ItemID].State ~= nil then
            ...
            if ChaStateLevel >= Server.Restore.Life[ItemID].StateLevel then
                SystemNotice(role, "You have used the same or higher state of food. Unable to use " ..
                    GetItemName(GetItemID(Item)) .. "!")
                UseItemFailed(role)
                return
            end
            AddState(role, role, Server.Restore.Life[ItemID].State, Server.Restore.Life[ItemID].StateLevel,
                Server.Restore.Life[ItemID].StateTime)
        end
...
function ItemStat(role, Item)
    ...
    if ItemCooldown[PlayerID][ItemID] ~= nil then
        ...
        if remainingCooldown > 0 then
            BickerNotice(role,
                string.format("Item:[%s] in cooldown mode, remaining time is %d sec(s)", GetItemName(GetItemID(Item)),
                    remainingCooldown))
            UseItemFailed(role)
            return
        end
    ...
    if (Stat + Server.Restore.Attr[ItemID].Amount) < 5 or (Stat + Server.Restore.Attr[ItemID].Amount) > 130 then
        SystemNotice(role,
            "You cannot use [" .. GetItemName(ItemID) .. "] to alter your [" .. Server.Restore.Attr[ItemID].Name ..
                "], since it will leave you below/above the limit of points.")
        UseItemFailed(role)
        return
    end

    SystemNotice(role,
        "You have successfully used [" .. GetItemName(ItemID) .. "] to alter your [" .. Server.Restore.Attr[ItemID].Name ..
            "] by " .. Server.Restore.Attr[ItemID].Amount .. " point(s).")
...
function BlackDragon_Altar(role, Item)
    ...
    if GetItemAttr(Item, ITEMATTR_URE) ~= 0 or GetItemAttr(Item, ITEMATTR_ENERGY) > Server.AltarBD.MaxCurse then
        SystemNotice(role, "There are too many curses or the seal on [" .. GetItemName(GetItemID(Item)) ..
    ...
    if GetChaFreeBagGridNum(role) <= 0 then
        SystemNotice(role,
            "You don't have free inventory slots, cannot unseal [" .. GetItemName(GetItemID(Item)) .. "] yet!")
        UseItemFailed(role)
        return
    end
    ...
    if GetItemAttr(Item, ITEMATTR_ENERGY) ~= 0 then
        SystemNotice(role, "The remaining " .. GetItemAttr(Item, ITEMATTR_ENERGY) ..
            " Curse Point(s)have damaged the quality of equipment obtained from [" .. GetItemName(GetItemID(Item)) ..
            "].")
    end
    ...
    Notice("[" .. GetItemName(GetItemID(Item)) .. "] has been unsealed! [" .. GetChaDefaultName(role) ..
               "] has obtained [" .. GetItemName(Equipment) .. "] with " .. GetItemAttr(Item, ITEMATTR_ENERGY) ..
               " Curse Point(s).")
...
function BlackDragon_Gem(role, Item, Target)
    if GetItemID(Target) ~= 266 or GetCtrlBoat(role) ~= nil then
        SystemNotice(role,
            "The sacrificial item [" .. GetItemName(GetItemID(Item)) .. "] can only be used [" .. GetItemName(266) ..
                "] while being in land!")
        UseItemFailed(role)
        return
    end
    if GetItemAttr(Target, ITEMATTR_VAL_STA) ~= 0 and Server.AltarBD.Trib[GetItemAttr(Target, ITEMATTR_VAL_STA)].Gem ==
        GetItemID(Item) then
        SystemNotice(role, "[" .. GetItemName(GetItemID(Target)) .. "] already has [" .. GetItemName(GetItemID(Item)) ..
            "] as a sacrificial item.")
        UseItemFailed(role)
        return
    end
    if GetItemType(Item) == 49 and GetItemType(Target) == 65 then
        for X = 1, 3, 1 do
            if Server.AltarBD.Trib[X].Gem == GetItemID(Item) then
                ...
                SystemNotice(role,
                    "You have used [" .. GetItemName(GetItemID(Item)) .. "] as a sacrificial item on [" ..
                        GetItemName(GetItemID(Target)) .. "], the result will be [" ..
                        GetItemName(Server.AltarBD.Trib[X].Equipment) .. "].")
...
function BlackDragon_Power(role, Item, Target)
    ...
    if GetItemType(Item) == 66 and GetItemType(Target) == 65 then
        ...
        if Percentage_Random(Server.AltarBD.PowerPercentage) == 1 then
            ...
            SetItemAttr(Target, Server.AltarBD.Power[GetItemID(Item)],
                (GetItemAttr(Target, Server.AltarBD.Power[GetItemID(Item)]) + 1))
        else
            SystemNotice(role, "Unfortunately, the [" .. GetItemName(GetItemID(Target)) ..
                "] has been cursed by the soul of the Black Dragon.")
...
function BlackDragon_Dice(role, Item, Target)
    if GetItemID(Target) ~= 266 or GetCtrlBoat(role) ~= nil then
        SystemNotice(role,
            "The cleansing item [" .. GetItemName(GetItemID(Item)) .. "] can only be used [" .. GetItemName(266) ..
                "] while being in land!")
        UseItemFailed(role)
        return
    end
    if GetItemAttr(Target, ITEMATTR_ENERGY) == 0 then
        SystemNotice(role,
            "There's no need to use the cleansing item [" .. GetItemName(GetItemID(Item)) .. "] on [" ..
                GetItemName(266) .. "], it has no Curse Point(s).")
        UseItemFailed(role)
        return
    end
    local Random = math.random(1, 100)
    for a = 1, #Server.AltarBD.Dice, 1 do
        if GetItemAttr(Target, ITEMATTR_ENERGY) >= Server.AltarBD.Dice[a].Min and GetItemAttr(Target, ITEMATTR_ENERGY) <=
            Server.AltarBD.Dice[a].Max then
            if Random <= Server.AltarBD.Dice[a].Percentage then
                SetItemAttr(Target, ITEMATTR_MAXENERGY, (GetItemAttr(Target, ITEMATTR_MAXENERGY) - 1))
                SetItemAttr(Target, ITEMATTR_ENERGY, (GetItemAttr(Target, ITEMATTR_ENERGY) - 1))
                SystemNotice(role, "The [" .. GetItemName(GetItemID(Item)) ..
                    "] you used was of a good quality, the curse was lifted.")
            else
                SystemNotice(role, "The [" .. GetItemName(GetItemID(Item)) ..
                    "] you used was of a bad quality, the curse was not lifted.")
            end
        end
    end
end
...
function Ticket(role, Item)
    ...
    if TicketSys.Ticket[ItemID].Level.Min ~= nil and GetChaAttr(role, ATTR_LV) < TicketSys.Ticket[ItemID].Level.Min then
        SystemNotice(role, "[" .. GetItemName(ItemID) .. "] is only usable for players above level " ..
            TicketSys.Ticket[ItemID].Level.Min .. ".")
        UseItemFailed(role)
        return
    end
    if TicketSys.Ticket[ItemID].Level.Max ~= nil and GetChaAttr(role, ATTR_LV) > TicketSys.Ticket[ItemID].Level.Max then
        SystemNotice(role, "[" .. GetItemName(ItemID) .. "] is only usable for players below level " ..
            TicketSys.Ticket[ItemID].Level.Max .. ".")
        UseItemFailed(role)
        return
    end
    if (Hp(role) < (Mxhp(role) * 0.5)) or (Sp(role) < (Mxsp(role) * 0.5)) or ChaIsBoat(role) == 1 then
        SystemNotice(role,
            "You must not be in a boat and have at least half of your maximum HP and SP in order to teleport.")
        UseItemFailed(role)
        return
    end
...
function NewbieChest(role, Item)
    ...
    if GetItemID(Item) == 437 then
        if Lv(role) < 10 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 9 then
            SystemNotice(role,
                "You need to be at least Level 10, have done First Class Advancement and have at least 9 free inventory slots.")
            UseItemFailed(role)
            return
        end
    ...
    if GetItemID(Item) == 438 then
        if Lv(role) < 15 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 6 then
            SystemNotice(role,
                "You need to be at least Level 15, have done First Class Advancement and have at least 6 free inventory slots.")
            UseItemFailed(role)
            return
        end
        if GetChaAttr(role, ATTR_JOB) == 1 or GetChaAttr(role, ATTR_JOB) == 8 or GetChaAttr(role, ATTR_JOB) == 9 then
    ...
    if GetItemID(Item) == 439 then
        if Lv(role) < 20 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 6 then
            SystemNotice(role,
                "You need to be at least Level 20, have done First Class Advancement and have at least 6 free inventory slots.")
            UseItemFailed(role)
            return
        end
        if GetChaAttr(role, ATTR_JOB) == 1 or GetChaAttr(role, ATTR_JOB) == 8 or GetChaAttr(role, ATTR_JOB) == 9 then
    ...
    if GetItemID(Item) == 440 then
        if Lv(role) < 25 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 5 then
            SystemNotice(role,
                "You need to be at least Level 25, have done First Class Advancement and have at least 5 free inventory slots.")
            UseItemFailed(role)
            return
        end
        if GetChaAttr(role, ATTR_JOB) == 1 or GetChaAttr(role, ATTR_JOB) == 8 or GetChaAttr(role, ATTR_JOB) == 9 then
    ...
    if GetItemID(Item) == 441 then
        if Lv(role) < 30 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 5 then
            SystemNotice(role,
                "You need to be at least Level 30, have done First Class Advancement and have at least 5 free inventory slots.")
            UseItemFailed(role)
            return
        end
        if GetChaAttr(role, ATTR_JOB) == 1 or GetChaAttr(role, ATTR_JOB) == 8 or GetChaAttr(role, ATTR_JOB) == 9 then
    ...
    if GetItemID(Item) == 442 then
        if Lv(role) < 35 or GetChaAttr(role, ATTR_JOB) == 0 or GetChaFreeBagGridNum(role) < 3 then
            SystemNotice(role,
                "You need to be at least Level 35, have done First Class Advancement and have at least 3 free inventory slots.")
            UseItemFailed(role)
            return
        end
    ...
    if GetItemID(Item) == 443 then
        if Lv(role) < 40 or GetChaFreeBagGridNum(role) < 4 or
            (GetChaAttr(role, ATTR_JOB) >= 0 and GetChaAttr(role, ATTR_JOB) <= 5) then
            SystemNotice(role,
                "You need to be at least Level 40 to open this chest, have promoted to Second Class Advancement and have 4 free inventory slots available.")
            UseItemFailed(role)
            return
        end
...
function Corsairs_WeaponChest(role, Item)
    ...
    Chest[322] = {
        ...
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Seal Master Chest of Magenta
    Chest[525] = {
        Active = 1,
        Item = 1375,
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Champion Chest of Magenta
    Chest[613] = {
        Active = 1,
        Item = 1394,
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Crusader Chest of Magenta
    Chest[614] = {
        Active = 1,
        Item = 0042,
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Sharpshooter Chest of Magenta
    Chest[615] = {
        Active = 1,
        Item = 1421,
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Voyager Chest of Magenta
    Chest[616] = {
        Active = 1,
        Item = 4198,
        Quantity = 1,
        Quality = 22
    } -- Lv 70 Cleric Chest of Magenta
    Chest[939] = {
        Active = 1,
        Item = 1392,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Crusader Chest of Magenta
    Chest[940] = {
        Active = 1,
        Item = 1373,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Champion Chest of Magenta
    Chest[941] = {
        Active = 1,
        Item = 0040,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Sharpshooter Chest of Magent
    Chest[942] = {
        Active = 1,
        Item = 1419,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Voyager Chest of Magenta
    Chest[943] = {
        Active = 1,
        Item = 0103,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Cleric Chest of Magenta
    Chest[944] = {
        Active = 1,
        Item = 0102,
        Quantity = 1,
        Quality = 22
    } -- Lv 50 Seal Master Chest of Magenta
    Chest[945] = {
        Active = 1,
        Item = 1393,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Crusader Chest of Magenta
    Chest[946] = {
        Active = 1,
        Item = 1374,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Champion Chest of Magenta
    Chest[947] = {
        Active = 1,
        Item = 0041,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Sharpshooter Chest of Magenta
    Chest[950] = {
        Active = 1,
        Item = 1420,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Voyager Chest of Magenta
    Chest[951] = {
        Active = 1,
        Item = 4303,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Cleric Chest of Magenta
    Chest[952] = {
        Active = 1,
        Item = 4300,
        Quantity = 1,
        Quality = 22
    } -- Lv 60 Seal Master Chest of Magenta
    local ItemID = GetItemID(Item)
...
function Corsairs_WeaponChest2(role, Item)
    ...
    Chest[6350] = {
        Active = 1,
        Item = {0004, 0015, 0039, 0076, 0100, 0101},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 40
    Chest[6351] = {
        Active = 1,
        Item = {0022, 0020, 0044, 0083, 4305, 0107},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 45
    Chest[6352] = {
        Active = 1,
        Item = {1392, 1373, 0040, 0077, 0103, 0102},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 50
    Chest[6353] = {
        Active = 1,
        Item = {0023, 0021, 0045, 0084, 1476, 0108},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 55
    Chest[6354] = {
        Active = 1,
        Item = {1393, 1374, 0041, 0078, 1442, 1439},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 60
    Chest[6355] = {
        Active = 1,
        Item = {4212, 4209, 4214, 4216, 4197, 4203},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 65
    Chest[6356] = {
        Active = 1,
        Item = {0113, 0115, 0119, 0150, 0109, 0111},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 70
    Chest[6357] = {
        Active = 1,
        Item = {0114, 0116, 0120, 0151, 0110, 0112},
        Quantity = 1,
        Quality = 22
    } -- Pirate Weapon Box Level 75
    ...
    if Chest[ItemID] ~= nil then
        ...
        GiveItem(role, 0, Chest[ItemID].Item[math.random(1, #Chest[ItemID].Item)], Chest[ItemID].Quantity,
            Chest[ItemID].Quality)
        SystemNotice(role, "Obtains " .. GetItemName(Chest[ItemID].Item) .. ".")
    end
end
function Corsairs_EquipmentChest(role, Item)
    ...
    Chest[617] = {
        ...
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 50 Crusader Chest	
    Chest[620] = {
        Active = 1,
        Item = {229, 653},
        Quantity = {1, 1},
        Quality = {22, 22}
    } -- Lv 50 Champion Chest
    Chest[623] = {
        Active = 1,
        Item = {371, 547, 723},
        Quantity = {1, 1, 1},
    ...
    Chest[626] = {
        Active = 1,
        Item = {382, 558, 734, 2204},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 50 Ami Cleric Chest			
    Chest[699] = {
        Active = 1,
        Item = {369, 545, 721},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 50 Seal Master Chest	
    Chest[629] = {
        Active = 1,
        Item = {385, 561, 737, 2207},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 50 Ami Seal Master Chest			
    Chest[676] = {
        Active = 1,
        Item = {345, 521, 697},
        Quantity = {1, 1, 1},
    ...
    Chest[632] = {
        Active = 1,
        Item = {355, 531, 707, 2192},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 50 Ami Voyager Chest			
    Chest[671] = {
        Active = 1,
        Item = {312, 488, 664},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 50 Sharpshooter Chest
    Chest[618] = {
        Active = 1,
        Item = {304, 480, 656},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 60 Crusader Chest
    Chest[621] = {
        Active = 1,
        Item = {230, 477},
        Quantity = {1, 1},
        Quality = {22, 22}
    } -- Lv 60 Champion Chest
    Chest[624] = {
        Active = 1,
        Item = {394, 570, 746},
        Quantity = {1, 1, 1},
    ...
    Chest[627] = {
        Active = 1,
        Item = {393, 569, 745, 2215},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 60 Ami Cleric Chest	
    Chest[700] = {
        Active = 1,
        Item = {377, 553, 729},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 60 Seal Master Chest	
    Chest[630] = {
        Active = 1,
        Item = {364, 540, 716, 2201},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 60 Ami Seal Master Chest
    Chest[686] = {
        Active = 1,
        Item = {344, 520, 696},
        Quantity = {1, 1, 1},
    ...
    Chest[633] = {
        Active = 1,
        Item = {358, 534, 710, 2195},
        Quantity = {1, 1, 1, 1},
        Quality = {22, 22, 22, 22}
    } -- Lv 60 Ami Voyager Chest
    Chest[673] = {
        Active = 1,
        Item = {317, 493, 669},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 60 Sharpshooter Chest
    Chest[619] = {
        Active = 1,
        Item = {4150, 4166, 4182},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Crusader Chest
    Chest[622] = {
        Active = 1,
        Item = {4148, 653, 477},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Champion Chest
    Chest[625] = {
        Active = 1,
        Item = {4159, 4175, 4191},
        Quantity = {1, 1, 1},
    ...
    Chest[628] = {
        ...
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Ami Cleric Chest
    Chest[701] = {
        Active = 1,
        Item = {4163, 4179, 4195},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Seal Master Chest
    Chest[631] = {
        Active = 1,
        Item = {4164, 4180, 4196},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Ami Seal Master Chest
    Chest[698] = {
        Active = 1,
        Item = {4155, 4171, 4187},
        Quantity = {1, 1, 1},
    ...
    Chest[634] = {
        ...
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Ami Voyager Chest
    Chest[674] = {
        Active = 1,
        Item = {4152, 4168, 4184},
        Quantity = {1, 1, 1},
        Quality = {22, 22, 22}
    } -- Lv 70 Sharpshooter Chest
    local ItemID = GetItemID(Item)
    if Chest[ItemID] ~= nil then
        local Item_CanGet = GetChaFreeBagGridNum(role)
...
function ItemUse_JLB_GoldenMap(role, Item)
    ...
    if Has_GoldenMap >= 2 then
        SystemNotice(role,
            "You can only have 1 treasure map in inventory to seek for treasure. Please deposit the rest into your vault")
        UseItemFailed(role)
        return
    end
    ...
    if GetPos == 0 then
        SystemNotice(role, "Treasure is hidden at " .. MapNameList[Themap] .. " Region. " .. pos_x .. "," .. pos_y ..
            " nearby")
        UseItemFailed(role)
        return
...
function GiveGoldenMapItem_JLB(role)
    ...
    elseif CheckRandom > 33 and CheckRandom <= 40 then
        ...
        if c ~= -1 then
            ...
            if GoodItem == 1 then
                local message = "" .. GetChaDefaultName(role) .. " Dug out a treasure and obtained " ..
                                    GetItemName(Item2Give) .. ""
                Notice(message)
            end
        end
    end
...
function ItemUse_minibh(role, Item)
    ...
    elseif star > 1 and star <= 51 then
        ...
        local message1 = cha_name ..
                             " opens a Mini Black Dragon Bag and obtained the First Prize: Happy Holiday Magazine"
        Notice(message1)
...
function ItemUse_YingbiBox(Player, Item)
    ...
    if chaLv < CoinChest.Conf.MinLv then
        SystemNotice(Player, "Character need to be Lv" .. CoinChest.Conf.MinLv .. "+ to open " ..
            GetItemName(CoinChest.Conf.ChestID) .. ".")
...
function ItemUse_ZBML1(role, Item)
    ...
    y = y_move + y
    local MonsterID = 947
    local Refresh = 700000 -- ֘ɺʱ¼䣬ëµ¥�?
    local life = 600000 -- ɺüʱ¼䣬ºÁëµ¥�?
    local new = CreateChaX(MonsterID, x, y, 145, Refresh, role)
    SetChaLifeTime(new, life)
end
-- Exploding Lamb Lv2-----------------------------------------------------------
...
function InventorySlot(role, Item)
    ...
    Inv[3088] = {
        Num = 24,
        Name = "28 Inventory Slot"
    }
    Inv[3089] = {
        Num = 28,
        Name = "32 Inventory Slot"
    }
    Inv[3090] = {
        Num = 32,
        Name = "36 Inventory Slot"
    }
    Inv[3091] = {
        Num = 36,
        Name = "40 Inventory Slot"
    }
    Inv[3092] = {
        Num = 40,
        Name = "44 Inventory Slot"
    }
    Inv[3093] = {
        Num = 44,
        Name = "48 Inventory Slot"
    }
    local ItemID = GetItemID(Item)
...
function Book_StatReset(role, Item)
    ...
    local STR, CON, AGI, ACC, SPR = GetChaAttr(role, ATTR_BSTR), GetChaAttr(role, ATTR_BCON),
        GetChaAttr(role, ATTR_BAGI), GetChaAttr(role, ATTR_BDEX), GetChaAttr(role, ATTR_BSTA)
    ...
    STR, CON, AGI, ACC, SPR = STR - PlayerStr + 5, CON - PlayerCon + 5, AGI - PlayerAgi + 5, ACC - PlayerAcc + 5,
        SPR - PlayerSpr + 5
...
function AddPlayerSkill(role, Item)
    ...
    if Server.Player.Skill[ItemID] ~= nil then
        ...
        if Skill == nil then
            SystemNotice(role, "Please contact administrator since " .. GetItemName(GetItemID(Item)) ..
...
function AddFairySkill(role, Item, Fairy)
    ...
    if Server.Fairy.Skill[ItemID] ~= nil then
        ...
        local FairyID = GetItemID(Fairy)
        local Check = CanLevelFairySkill(role, Fairy, Item, Server.Fairy.Skill[ItemID].Skill,
            Server.Fairy.Skill[ItemID].Level)
        if GetItemType(Item) == 58 and GetItemType(Fairy) == 59 then
            if not Check then
                UseItemFailed(role)
                return
                ...
                RefreshCha(role)
                LG("FairySkills", "" .. GetChaDefaultName(role) .. " used " .. GetItemName(ItemID) .. " for his/her " ..
                    GetItemName(FairyID) .. ".")
            end
            SynLook(role)
        else
...
function AddLifeSkill(role, Item)
    ...
    if Server.Life.Skill[ItemID] ~= nil then
        ...
        if Server.Life.Skill[ItemID].Prerequisite.Skill ~= nil then
            if GetSkillLv(role, Server.Life.Skill[ItemID].Prerequisite.Skill) <
                Server.Life.Skill[ItemID].Prerequisite.Level then
                SystemNotice(role, "Requires you to learn Lv" .. Server.Life.Skill[ItemID].Prerequisite.Level .. " " ..
                    GetSkillName(Server.Life.Skill[ItemID].Prerequisite.Skill) .. " skill to read this book")
                UseItemFailed(role)
                return
            end
        end
...
function AddFairyBody(role, Item)
    ...
    if Server.Fairy.Possession.Skill[ItemID] ~= nil then
        ...
        local AddSkill = AddChaSkill(role, Server.Fairy.Possession.Skill[ItemID].Skill,
            Server.Fairy.Possession.Skill[ItemID].Level, 1, 0)
        if AddSkill == 0 then
            SystemNotice(role, "Internal error!")
            UseItemFailed(role)
            return
...
function AddFunnySkill(role, Item)
    ...
    if Server.Fairy.Funny.Skill[ItemID] ~= nil then
        ...
        local AddSkill = AddChaSkill(role, Server.Fairy.Funny.Skill[ItemID].Skill,
            Server.Fairy.Funny.Skill[ItemID].Level, 1, 0)
        if AddSkill == 0 then
            SystemNotice(role, "Internal error!")
            UseItemFailed(role)
            return
...
function BlackMarket(role, Item)
    ...
    if Market[ItemID] ~= nil then
        ...
        if c ~= -1 then
            ...
            if GoodItem == 1 then
                local message =
                    "" .. GetChaDefaultName(role) .. " opens a " .. GetItemName(ItemID) .. " and obtains " ..
                        GetItemName(Item2Give) .. ""
                Notice(message)
            end
        end
    ...
    if vItem[ItemID] ~= nil then
        ...
        if Lucky == 500 then
            ...
            if c ~= -1 then
                ...
                GiveItem(role, 0, Item2Give, ItemCount, ItemQuality)
                if GoodItem == 1 then
                    local message = "" .. GetChaDefaultName(role) .. " opens a " .. GetItemName(ItemID) ..
                                        " and obtains " .. GetItemName(Item2Give) .. ""
                    Notice(message)
                end
            end
        end
...
function AzzGemP(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
-- Chest of Great Gems
function GreatGemP(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AdvGemP(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
-- Chest of Jade Gems
function JadeGemP(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function ManuPacks(role, Item) ----------真黑龙宝�? 
    ...
    if IsInTeam(role) == 1 then
        ...
        for i = 1, #Team do
            ...
            if Boat == nil then
                ...
                SystemNotice(Team[i], GetChaDefaultName(role) .. " has consumed " .. GetItemName(GetItemID(Item)) ..
                    ". Received 1.5x Team Experience bonus for 15 mins.")
            else
                AddState(Boat, Boat, STATE_ZDSBJYGZ, StateLevel, StateTime)
                SystemNotice(Boat, GetChaDefaultName(role) .. " has consumed " .. GetItemName(GetItemID(Item)) ..
                    ". Received 1.5x Team Experience bonus for 15 mins.")
            end
        end
    else
...
function Corsairs_WangChest(role, Item)
    ...
    if Level >= 50 and Level <= 55 then
        if Chance == 1 then
            ...
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Champion's Belt]",
                1)
            return
        elseif Chance == 2 then
            GiveItem(role, 0, 7297, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Crusader's Belt]",
                1)
            return
        elseif Chance == 3 then
            GiveItem(role, 0, 7300, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Cleric's Belt]",
                1)
            return
        elseif Chance == 4 then
            GiveItem(role, 0, 7303, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Seal Master's Belt]",
                1)
            return
        elseif Chance == 5 then
            GiveItem(role, 0, 7306, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Voyager's Belt]",
                1)
            return
        elseif Chance == 6 then
            GiveItem(role, 0, 7309, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv55 Sharpshooter's Belt]",
                1)
            return
        end
    end
    if Level >= 60 then
        if Chance == 1 then -- Lv65 Belt
            ...
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Champion's Belt]",
                1)
            return
        elseif Chance == 2 then
            GiveItem(role, 0, 7315, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Crusader's Belt]",
                1)
            return
        elseif Chance == 3 then
            GiveItem(role, 0, 7318, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Cleric's Belt]",
                1)
            return
        elseif Chance == 4 then
            GiveItem(role, 0, 7321, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Seal Master's Belt]",
                1)
            return
        elseif Chance == 5 then
            GiveItem(role, 0, 7324, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Voyager's Belt]",
                1)
            return
        elseif Chance == 6 then
            GiveItem(role, 0, 7327, 1, 4)
            Notice("Congratulations to player " .. Name ..
                       " for defeating the Thief - Wang Xiao Hu and obtained [Wang Xiao Hu's Chest]. Wow, you're lucky to receive a [Lv65 Sharpshooter's Belt]",
                1)
            return
        end
    end
...
function AccCruss(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AccChamp(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AccSharp(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AccVoy(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AccCler(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function AccSeal(role, Item) ----------真黑龙宝�? 
    if KitbagLock(role, 0) == LUA_FALSE then
        PopupNotice(role, "Inventory is locked.")
        return
    end
...
function ItemUse_FlyWings(role, Item, Item_Traget)
    ...
    SystemNotice(role, 'Successfully upgrade ' .. GetItemName(GetItemID(Item_Traget)) ..
        '! You can now use your wings to fly')
...
function ItemUse_Admiral(role, Item)
    ...
    if GetChaFreeBagGridNum(role) < 2 then
        ...
        ScrollNotice("Congratulations! role " .. GetChaDefaultName(role) .. " has obtained Lv1 Admiral Cloak", 1,
            0XFFF6E58D)
    end
end
...
function Cloak_Upgrade(role, Item, Target)
    ...
    if Percentage_Random(Server.Cloak.Rate[Level]) == 1 then
        ...
        SynChaKitbag(role, 13)
        SynLook(role)
        SystemNotice(role, "Congratulations! You have succesfully enchanced your Admiral Cloak to LV " ..
            math.floor(Level + 1) .. ".")
    else
        SystemNotice(role,
            "Sorry, upgrade has failed! Luckily [" .. GetItemName(GetItemID(Target)) .. "] is not damaged...")
        PlayEffect(role, 346)
    end
end
...
function ItemUse_Nightmare(role, Item)
    ...
    if GetChaFreeBagGridNum(role) < 2 then
        ...
        Notice(GetChaDefaultName(role) .. ' successfully Unsealed a ' .. GetItemName(ItemID) .. '!');
        ScrollNotice("Congratulations! role " .. GetChaDefaultName(role) .. " has obtained Lv1 Nightmare Demonic Helm",
            1, 0XFFF6E58D)
    end
end
...
function Nightmare_Demonic(role, Item, Target)
    ...
    if r == 1 then
        ...
        LG('Nightmare Demonic', 'Player [' .. GetChaDefaultName(role) .. '] Successfully upgrade [ ' .. itemname ..
            '! to lvl' .. Level .. "]")
        PlayEffect(role, 345)
        ...
        if Level == 10 then
            ...
            LG('Nightmare Demonic Helm Max',
                '' .. GetChaDefaultName(role) .. ' has maxed out the level of Nightmare Demonic Helm.')
        end
        SynChaKitbag(role, 13)
        ...
        PlayEffect(role, 346)
        LG('Nightmare Demonic',
            'Player [' .. GetChaDefaultName(role) .. '] Failed upgrade [ ' .. itemname .. '! to lvl' .. Level .. "]")
    end
end
