...
function Give_ElfEXP(Player,givenExp,maxexp)
    local Fairy = GetEquipItemP(Player, 16)
	if Fairy == nil then
        SystemNotice(Player, "Please equip your fairy first, then try again.")
		return LUA_FALSE
	end
    --this means we give range exp value 
    if maxexp then
        givenExp = math.random(givenExp,maxexp) 
    end
    local FairyEXP = GetItemAttr(Fairy, ITEMATTR_ENERGY)
    local FairyMaxEXP = GetItemAttr(Fairy, ITEMATTR_MAXENERGY)
    if GetItemAttr(Fairy, ITEMATTR_URE) > 49 then
        FairyEXP = FairyEXP + givenExp--Server.Rates.Global.FairyEXP
        if FairyEXP >= FairyMaxEXP then
            FairyEXP = FairyMaxEXP
        end
        SystemNotice(Player, "Fairy EXP " .. givenExp .. " has been added to your fairy. Total EXP now: " .. FairyEXP .. ".")  
        SetItemAttr(Fairy, ITEMATTR_ENERGY, FairyEXP)
        RefreshCha(Player)
    end
    SynLook(Player)
...
local DesiredGemItemType = GetItemType(DesiredGem)
	...
	PlayEffect(role, 345);
	LG( "Phoenix Gem Scroll" , "Player: ["..CharacterName.."] used Phoenix Scroll and combined Lv["..DesiredGemLv.."] ["..GetItemName(DesiredGemID).."] ["..DesiredGemID.."]")
