...
function init_entry(map)
	...
	--SetMapEntryTime(map, "2006/10/18/1/0", "0/12/0", "0/0/30", "0/0/45") -- server official time 9 pm and 9 am every 12 hours
	SetMapEntryTime(map, "2006/10/18/5/0", "0/12/0", "0/0/30", "0/0/45")
end
...
function Garner2_Init()
    PANEL_PLAYER_KILL_COUNT["garner2"] = {}
    PANEL_PLAYER_DEATH_COUNT["garner2"] = {}
	PANEL_LAST_PLAYER_KILLED["garner2"] = {}
	Garner2_Active_Player = 0
	Garner2_Money = 10000000
	Garner2_Wave1 = 0
	Garner2_Wave2 = 0
	Garner2_Wave3 = 0
	Garner2_Wave4 = 0
...
function after_enter_garner2(Player, map_copy)
	...
	Restriction.Add(Player, 'garner2', 'MAC')
	
	--UpdateTeamPointsPanel(Player, "Players alive: \nYour kills: \nYour deaths: ")
	
	-- local GuildID = GetChaGuildID(Player)
	-- if GuildID ~= 0 then
		-- Garner2_GuildLog[GuildID] = Garner2_GuildLog[GuildID] or 0
		-- Garner2_GuildLog[GuildID] = Garner2_GuildLog[GuildID] + 1
	-- else 
		-- Garner2_GuildLog[0] = Garner2_GuildLog[0] or 0
		-- Garner2_GuildLog[0] = Garner2_GuildLog[0] + 1
	-- end
end

function before_leave_garner2(Player)
	Notice(GetChaDefaultName(Player)..' has left Chaos Argent!')
	Restriction.Rem(Player, 'garner2', 'MAC')
	...
	-- if GuildID ~= 0 then
		...
		-- if Garner2_GuildLog[GuildID] <= 0 then
			-- Garner2_GuildLog[GuildID] = nil
		-- end
	-- else 
		-- Garner2_GuildLog[0] = Garner2_GuildLog[0] or 0
		-- Garner2_GuildLog[0] = Garner2_GuildLog[0] - 1
		
		-- if Garner2_GuildLog[0] <= 0 then
			-- Garner2_GuildLog[0] = nil
		-- end
	-- end
end
...
function map_copy_run_garner2(map_copy)
	...
	local MapName = "Chaos Argent"
	SetMapEntryEventName(PortalEntryData.garner2, ""..MapName.." | [ "..Survivor.." ] Survivor(s)")
	local Hour = tonumber(os.date("%H"))
	local Minute = tonumber(os.date("%M"))
	local Second = tonumber(os.date("%S"))
	local Message = "Chaos Argent still have survivor(s): "..Survivor.." player(s)!"

	DealAllPlayerInMap(map_copy,"Garner2UpdateKdrPanel")
	if Hour == 5 or Hour == 17 then
		...
		GMNotice(Message)
			if Garner2_Wave7 == 0 then
				...
				local Monster334 = CreateChaEx(1842, 13575, 14062, 205, 60, map_copy)
		...
		if Minute >= 40 and Minute < 45 then
			...
			if Garner2_Tick == 60 then
				ScrollNotice("Chaos Argent will be closed in 60 seconds - "..Survivor.." Survivor(s) !",1, 0XFFF6E58D)
			end
...
function ChaosArgentRewards(Player)
	...
	if Garner2_Active_Player > 5 then
		...
		if Garner2_Active_Player <  6 and Garner2_Active_Player > 1 then
	if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9982, 1, 4) --Saint Of Argent
			...
			GiveItemX(Player, 0, 9982, 1, 4) --Saint Of Argent
	...
	if Garner2_Active_Player == 1 then
		if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9978, 1, 4)	--King Of Argent
			GiveItem(Player, 0, 3878, 2, 4) --Chest of Kal Runestone
			GiveItem(Player, 0, 9941, 3, 4) --Random Azrael Gem Chest
			GiveItem(Player, 0, 9612, 3, 4)	--Great Gem Pack
		else
			GiveItemX(Player, 0, 9978, 1, 4)
			GiveItemX(Player, 0, 3878, 2, 4)
			GiveItemX(Player, 0, 9941, 3, 4)
			GiveItemX(Player, 0, 9612, 3, 4)
		end
		AddMoney(Player, 0, Money)
		ScrollNotice("[Chaos Argent]: Congratulations "..GetChaDefaultName(Player)..", you are the winner of Chaos Argent on this round!",1, 0XFFF6E58D)
	return
	end
end

--- Function to get the player with the most kills in the "garner2" map
---@return string player_name The name of the player with the most kills, or "N/A" if no players have kills
---@return string|integer player_name and most_kills name of player and num of kills 
function Garner2GetReaperPlayer()
    local garner2_kills = PANEL_PLAYER_KILL_COUNT["garner2"]
    if not garner2_kills then
        return "N/A", 0
    end
    ...
    for player_name, kills in pairs(garner2_kills) do
...
-- at end of ctrl.lua
function Garner2UpdateKdrPanel(character)
    local map_copy = GetChaMapCopy(character)
    --local survivor = GetMapActivePlayer(map_copy)
    local character_name = GetChaDefaultName(character)

    -- Ensure the "garner2" map entry exists in PANEL_PLAYER_KILL_COUNT
	PANEL_PLAYER_KILL_COUNT["garner2"] = PANEL_PLAYER_KILL_COUNT["garner2"] or {}
	PANEL_PLAYER_DEATH_COUNT["garner2"] = PANEL_PLAYER_DEATH_COUNT["garner2"] or {}
	PANEL_LAST_PLAYER_KILLED["garner2"] = PANEL_LAST_PLAYER_KILLED["garner2"] or {}

    -- Now safely access the character kills, deaths and last player killed
    local character_kills = PANEL_PLAYER_KILL_COUNT["garner2"][character_name] or 0
    local character_deaths = PANEL_PLAYER_DEATH_COUNT["garner2"][character_name] or 0
    local last_player_killed = GetChaDefaultName(PANEL_LAST_PLAYER_KILLED["garner2"][character_name]) or "N/A"
	local chaos_reaper_player_name, chaos_reaper_kills = Garner2GetReaperPlayer()
	local survivor = GetMapActivePlayer(map_copy)
...
-- Garner2Notice = function(map_copy)
	-- local Message = '[Chaos Argent]: Total Players Alive: '..GetMapActivePlayer(map_copy)
	-- if table.getn(Garner2_GuildLog) ~= 0 then
		-- for GuildID, GuildCount in pairs(Garner2_GuildLog) do
		-- if GuildID == 0 then
