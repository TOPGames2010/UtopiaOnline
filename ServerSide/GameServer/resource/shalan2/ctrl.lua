print("------------------------------------")
print("-- Loading Chaos Desert Pk file            --")
print("------------------------------------")
...
function init_entry(map)
    SetMapEntryMapName(map, "lobbycityafk")
   	--SetMapEntryTime(map, "2006/10/18/4/0", "0/12/0", "0/0/30", "0/0/45")-- server official time 12 pm and 12 am every 12 hours
	SetMapEntryTime(map, "2006/10/18/2/0", "0/12/0", "0/0/30", "0/0/45")
end

function Shalan2_Init()
	PANEL_PLAYER_KILL_COUNT["shalan2"] = {}
    PANEL_PLAYER_DEATH_COUNT["shalan2"] = {}
	PANEL_LAST_PLAYER_KILLED["shalan2"] = {}
	Shalan2_Active_Player = 0
	Shalan2_Money = 10000000
	Shalan2_Wave1 = 0
	Shalan2_Wave2 = 0
	Shalan2_Wave3 = 0
	Shalan2_Wave4 = 0
...
function after_enter_shalan2( Player , map_copy )
	Restriction.Add(Player, 'shalan2', 'MAC')
	
	-- local GuildID = GetChaGuildID(Player)
	-- if GuildID ~= 0 then
		-- Shalan2_GuildLog[GuildID] = Shalan2_GuildLog[GuildID] or 0
		-- Shalan2_GuildLog[GuildID] = Shalan2_GuildLog[GuildID] + 1
	-- else 
		-- Shalan2_GuildLog[0] = Shalan2_GuildLog[0] or 0
		-- Shalan2_GuildLog[0] = Shalan2_GuildLog[0] + 1
	-- end
end
function before_leave_shalan2( Player )
	Notice(GetChaDefaultName(Player)..' has left Chaos Desert!')
	Restriction.Rem(Player, 'shalan2', 'MAC')
	...
	-- if GuildID ~= 0 then
		...
		-- if Shalan2_GuildLog[GuildID] <= 0 then
			-- Shalan2_GuildLog[GuildID] = nil
		-- end
		-- else 
		-- Shalan2_GuildLog[0] = Shalan2_GuildLog[0] or 0
		-- Shalan2_GuildLog[0] = Shalan2_GuildLog[0] - 1
		
		-- if Shalan2_GuildLog[0] <= 0 then
			-- Shalan2_GuildLog[0] = nil
		-- end
	-- end
end
...
function map_copy_run_shalan2( map_copy )
	...
	local MapName = "Chaos Desert"
	SetMapEntryEventName(PortalEntryData5.shalan2, ""..MapName.." | [ "..Survivor.." ] Survivor(s)")
	local Hour = tonumber(os.date("%H"))
	local Minute = tonumber(os.date("%M"))
	local Second = tonumber(os.date("%S"))
	local Message = "Chaos Desert still have survivor(s): "..Survivor.." player(s)!"
	--ScrollNotice ("Chaos Glacier still have survivor(s): "..Survivor.." player(s)!",1, 0XFFF6E58D)
	DealAllPlayerInMap(map_copy,"Shalan2UpdateKdrPanel")
	if Hour == 2 or Hour == 14 then
		if Minute == 5 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave1 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave1 = Shalan2_Wave1 + 1
			end
		end
		if Minute == 10 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave2 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave2 = Shalan2_Wave2 + 1
			end
		end
		if Minute == 15 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave3 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave3 = Shalan2_Wave3 + 1
			end
		end
		if Minute == 20 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave4 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave4 = Shalan2_Wave4 + 1
			end
		end
		if Minute == 25 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave5 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave5 = Shalan2_Wave5 + 1
			end
		end
		if Minute == 30 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave6 == 0 then
				...
				--GMNotice(Message)
				Shalan2_Wave6 = Shalan2_Wave6 + 1
			end
		end
		if Minute == 35 and Second == 0 then
		GMNotice(Message)
			if Shalan2_Wave7 == 0 then
				...
				local Monster334 = CreateChaEx(1841, 11100, 10225, 205, 60, map_copy)
				...
				--GMNotice(Message)
				Shalan2_Wave7 = Shalan2_Wave7 + 1
			end
		end
		if Minute >= 40 and Minute < 45 then
			if Shalan2_Tick == 300 then
				Notice("Chaos Desert will be closed in 5 minutes - "..Survivor.." Survivor(s) !")
			end
			if Shalan2_Tick == 180 then
				Notice("Chaos Desert will be closed in 3 minutes - "..Survivor.." Survivor(s) !")
			end
			if Shalan2_Tick == 120 then
				Notice("Chaos Desert will be closed in 2 minutes - "..Survivor.." Survivor(s) !")
			end
			if Shalan2_Tick == 60 then
				ScrollNotice("Chaos Desert will be closed in 60 seconds - "..Survivor.." Survivor(s) !",1, 0XFFF6E58D)
			end
			Shalan2_Tick = Shalan2_Tick - 1
		end
	end
end
...
function map_copy_before_close_shalan2( map_copy )
	Shalan2_Active_Player = GetMapActivePlayer(map_copy)
	if Shalan2_Active_Player > 0 then
		DealAllActivePlayerInMap(map_copy, "ChaosDesertRewards")
	end
	Shalan2_Init()
end
...
function ChaosDesertRewards(Player)
	if Shalan2_Active_Player == 0 then
		return
	end
	local Money = math.floor(Shalan2_Money/Shalan2_Active_Player)
	if Shalan2_Active_Player > 5 then
		AddMoney(Player, 0, Money)
		return
	end
	if Shalan2_Active_Player <  6 and Shalan2_Active_Player > 1 then
	if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9975, 1, 4) --Saint Of Desert
			GiveItem(Player, 0, 3457, 5, 4)
			GiveItem(Player, 0, 0854, 3, 4)
			GiveItem(Player, 0, 9941, 1, 4)
			GiveItem(Player, 0, 9612, 1, 4)
		else
			GiveItemX(Player, 0, 9975, 1, 4) --Saint Of Desert
	...
	----------------------------------
	if Shalan2_Active_Player == 1 then
		if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9981, 1, 4)		--King Of Desert
			GiveItem(Player, 0, 3878, 1, 4)
			GiveItem(Player, 0, 9941, 5, 4)
			GiveItem(Player, 0, 9612, 3, 4)		
		else
			GiveItemX(Player, 0, 9981, 1, 4) 	--King Of Desert
			GiveItemX(Player, 0, 3878, 1, 4)
			GiveItemX(Player, 0, 9941, 5, 4)
			GiveItemX(Player, 0, 9612, 3, 4)
		end
		AddMoney(Player, 0, Money)
		ScrollNotice("[Chaos Desert]: Congratulations "..GetChaDefaultName(Player)..", you are the winner of Chaos Desert on this round!",1, 0XFFF6E58D)
	return	
	end
end
function Shalan2GetReaperPlayer()
    local shalan2_kills = PANEL_PLAYER_KILL_COUNT["shalan2"]
    if not shalan2_kills then
        return "N/A", 0
    end
    ...
    for player_name, kills in pairs(shalan2_kills) do
...
---- KDR PANEL
function Shalan2UpdateKdrPanel(character)
    local map_copy = GetChaMapCopy(character)
    --local survivor = GetMapActivePlayer(map_copy)
    local character_name = GetChaDefaultName(character)

    -- Ensure the "shalan2" map entry exists in PANEL_PLAYER_KILL_COUNT
	PANEL_PLAYER_KILL_COUNT["shalan2"] = PANEL_PLAYER_KILL_COUNT["shalan2"] or {}
	PANEL_PLAYER_DEATH_COUNT["shalan2"] = PANEL_PLAYER_DEATH_COUNT["shalan2"] or {}
	PANEL_LAST_PLAYER_KILLED["shalan2"] = PANEL_LAST_PLAYER_KILLED["shalan2"] or {}

    -- Now safely access the character kills, deaths and last player killed
    local character_kills = PANEL_PLAYER_KILL_COUNT["shalan2"][character_name] or 0
    local character_deaths = PANEL_PLAYER_DEATH_COUNT["shalan2"][character_name] or 0
    local last_player_killed = GetChaDefaultName(PANEL_LAST_PLAYER_KILLED["shalan2"][character_name]) or "N/A"
	local chaos_reaper_player_name, chaos_reaper_kills = Shalan2GetReaperPlayer()
	local survivor = GetMapActivePlayer(map_copy)
...
-- Shalan2Notice = function(map_copy)
	-- local Message = '[Chaos Desert]: Total Players Alive: '..GetMapActivePlayer(map_copy)
	-- if table.getn(Shalan2_GuildLog) ~= 0 then
		-- for GuildID, GuildCount in pairs(Shalan2_GuildLog) do
		-- if GuildID == 0 then
