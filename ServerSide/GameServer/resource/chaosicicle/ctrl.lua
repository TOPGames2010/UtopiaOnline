print("------------------------------------")
print("-- Loading chaosicicle file            --")
print("------------------------------------")
...
function get_map_entry_pos_chaosicicle()
	return 282,373
end

function init_entry(map)
    SetMapEntryMapName(map, "lobbycityafk")
	--SetMapEntryTime(map, "2006/10/18/2/0", "0/12/0", "0/0/30", "0/0/45") -- server official time 10 pm and 10 am every 12 hours
	SetMapEntryTime(map, "2006/10/18/9/0", "0/12/0", "0/0/30", "0/0/45")

end
function chaosicicle_Init()
    PANEL_PLAYER_KILL_COUNT["chaosicicle"] = {}
    PANEL_PLAYER_DEATH_COUNT["chaosicicle"] = {}
	PANEL_LAST_PLAYER_KILLED["chaosicicle"] = {}
	chaosicicle_Active_Player = 0
	chaosicicle_Money = 10000000
	chaosicicle_Wave1 = 0
	chaosicicle_Wave2 = 0
	chaosicicle_Wave3 = 0
	chaosicicle_Wave4 = 0
	...
	chaosicicle_Wave6 = 0
	chaosicicle_Wave7 = 0
	chaosicicle_Tick = 300
	--chaosicicle_GuildLog = {}
end
chaosicicle_Init()

function after_enter_chaosicicle( Player , map_copy )
	Restriction.Add(Player, 'chaosicicle', 'MAC')
	-- local GuildID = GetChaGuildID(Player)
	-- if GuildID ~= 0 then
		-- chaosicicle_GuildLog[GuildID] = chaosicicle_GuildLog[GuildID] or 0
		-- chaosicicle_GuildLog[GuildID] = chaosicicle_GuildLog[GuildID] + 1
	-- else 
		-- chaosicicle_GuildLog[0] = chaosicicle_GuildLog[0] or 0
		-- chaosicicle_GuildLog[0] = chaosicicle_GuildLog[0] + 1
	-- end
end

function before_leave_chaosicicle( Player )
	Notice(GetChaDefaultName(Player)..' has left Chaos Icicle!')
	Restriction.Rem(Player, 'chaosicicle', 'MAC')
	...
	-- if GuildID ~= 0 then
		...
		-- if chaosicicle_GuildLog[GuildID] <= 0 then
			-- chaosicicle_GuildLog[GuildID] = nil
		-- end
	-- else 
		-- chaosicicle_GuildLog[0] = chaosicicle_GuildLog[0] or 0
		-- chaosicicle_GuildLog[0] = chaosicicle_GuildLog[0] - 1
		
		-- if chaosicicle_GuildLog[0] <= 0 then
			-- chaosicicle_GuildLog[0] = nil
		-- end
	-- end
end
...
function map_copy_run_chaosicicle( map_copy )
	...
	local MapName = "Chaos Icicle"
	SetMapEntryEventName(PortalEntryData2.chaosicicle, ""..MapName.." | [ "..Survivor.." ] Survivor(s)")
	local Hour = tonumber(os.date("%H"))
	local Minute = tonumber(os.date("%M"))
	local Second = tonumber(os.date("%S"))
	local Message = "Chaos Icicle still have survivor(s): "..Survivor.." player(s)!"
	
	DealAllPlayerInMap(map_copy,"ChaosIciUpdateKdrPanel")
	
	if Hour == 9 or Hour == 21 then
		if Minute == 5 and Second == 0 then
		GMNotice(Message)
		if chaosicicle_Wave1 == 0 then
		local Monster2 = CreateChaEx(859, 9400, 8400, 145, 300,map_copy)
		local Monster3 = CreateChaEx(859, 15700, 9000, 145, 300,map_copy)
		local Monster4 = CreateChaEx(859, 18100, 11100, 145, 300,map_copy)
		local Monster5 = CreateChaEx(859, 11500, 12300, 145, 300,map_copy)
		local Monster6 = CreateChaEx(859, 8000, 16200, 145, 300,map_copy)
		local Monster7 = CreateChaEx(859, 15800, 15800, 145, 300,map_copy)
		local Monster8 = CreateChaEx(859, 12600, 18000, 145, 300,map_copy)
		local Monster9 = CreateChaEx(859, 13000, 20500, 145, 300,map_copy)
		local Monster10 = CreateChaEx(859, 17500, 21000, 145, 300,map_copy)
		local Monster11 = CreateChaEx(859, 7800, 222000, 145, 300,map_copy)
		...
		--GMNotice(Message)
		chaosicicle_Wave1 = chaosicicle_Wave1 + 1
	end
		end
	if Minute == 10 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave2 == 0 then
		local Monster12 = CreateChaEx(860, 9400, 8400, 145, 300,map_copy)
		local Monster13 = CreateChaEx(860, 15700, 9000, 145, 300,map_copy)
		local Monster14 = CreateChaEx(860, 18100, 11100, 145, 300,map_copy)
		local Monster15 = CreateChaEx(860, 11500, 12300, 145, 300,map_copy)
		local Monster16 = CreateChaEx(860, 8000, 16200, 145, 300,map_copy)
		local Monster17 = CreateChaEx(860, 15800, 15800, 145, 300,map_copy)
		local Monster18 = CreateChaEx(860, 12600, 18000, 145, 300,map_copy)
		local Monster19 = CreateChaEx(860, 13000, 20500, 145, 300,map_copy)
		local Monster20 = CreateChaEx(860, 17500, 21000, 145, 300,map_copy)
		local Monster21 = CreateChaEx(860, 7800, 222000, 145, 300,map_copy)
		...
		--GMNotice(Message)
		chaosicicle_Wave2 = chaosicicle_Wave2 + 1
			end
	end
	if Minute == 15 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave3 == 0 then
		local Monster22 = CreateChaEx(861, 9400, 8400, 145, 300,map_copy)
		local Monster23 = CreateChaEx(861, 15700, 9000, 145, 300,map_copy)
		local Monster24 = CreateChaEx(861, 18100, 11100, 145, 300,map_copy)
		local Monster25 = CreateChaEx(861, 11500, 12300, 145, 300,map_copy)
		local Monster26 = CreateChaEx(861, 8000, 16200, 145, 300,map_copy)
		local Monster27 = CreateChaEx(861, 15800, 15800, 145, 300,map_copy)
		local Monster28 = CreateChaEx(861, 12600, 18000, 145, 300,map_copy)
		local Monster29 = CreateChaEx(861, 13000, 20500, 145, 300,map_copy)
		local Monster30 = CreateChaEx(861, 17500, 21000, 145, 300,map_copy)
		local Monster31 = CreateChaEx(861, 7800, 222000, 145, 300,map_copy)
		...
		--GMNotice(Message)
		chaosicicle_Wave3 = chaosicicle_Wave3 + 1
			end
	end
	if Minute == 20 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave4 == 0 then
		...
		--GMNotice(Message)
		chaosicicle_Wave4 = chaosicicle_Wave4 + 1
			end
	end
	if Minute == 25 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave5 == 0 then
		...
		local Monster79 = CreateChaEx(873, 15500, 22500, 145, 60,map_copy)
			...
			--GMNotice(Message)
			chaosicicle_Wave5 = chaosicicle_Wave5 + 1
			end
	end
	if Minute == 30 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave6 == 0 then
		...
		local Monster167 = CreateChaEx(883, 10394, 6937, 145, 60,map_copy)
			...
			--GMNotice(Message)
			chaosicicle_Wave6 = chaosicicle_Wave6 + 1
			end
	end
	if Minute == 35 and Second == 0 then
	GMNotice(Message)
		if chaosicicle_Wave7 == 0 then
		...
		local Monster334 =CreateChaEx(1843, 7062, 12487, 205, 60,map_copy)
			...
			--GMNotice(Message)
			chaosicicle_Wave7 = chaosicicle_Wave7 + 1
		end
	end
	if Minute >= 40 and Minute < 45 then
			if chaosicicle_Tick == 300 then
				Notice("Chaos Icicle will be closed in 5 minutes - "..Survivor.." Survivor(s) !")
			end
			if chaosicicle_Tick == 180 then
				Notice("Chaos Icicle will be closed in 3 minutes - "..Survivor.." Survivor(s) !")
			end
			if chaosicicle_Tick == 120 then
				Notice("Chaos Icicle will be closed in 2 minutes - "..Survivor.." Survivor(s) !")
			end
			if chaosicicle_Tick == 60 then
				ScrollNotice("Chaos Icicle will be closed in 60 seconds - "..Survivor.." Survivor(s) !",1, 0XFFF6E58D)
			end
			chaosicicle_Tick = chaosicicle_Tick - 1
		end
	end
end
...
function map_copy_before_close_chaosicicle( map_copy )
	chaosicicle_Active_Player = GetMapActivePlayer(map_copy)
	if chaosicicle_Active_Player > 0 then
		DealAllActivePlayerInMap(map_copy, "ChaosIcicleRewards")
	end
	chaosicicle_Init()
end
...
function ChaosIcicleRewards(Player)
	if chaosicicle_Active_Player == 0 then
		return
	end
	local Money = math.floor(chaosicicle_Money/chaosicicle_Active_Player)
	if chaosicicle_Active_Player > 5 then
		AddMoney(Player, 0, Money)
		return
	end
	if chaosicicle_Active_Player <  6 and chaosicicle_Active_Player > 1 then
		if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9983, 1, 4) --Saint Of Argent
			...
			GiveItemX(Player, 0, 9983, 1, 4) --Saint Of Icicle
	...
	if chaosicicle_Active_Player == 1 then
		if GetChaFreeBagGridNum(Player) >= 5 then
			GiveItem(Player, 0, 9979, 1, 4)		--King Of Icicle
			...
			GiveItemX(Player, 0, 9979, 1, 4) 	--King Of Icicle
			GiveItemX(Player, 0, 3878, 1, 4)
			GiveItemX(Player, 0, 9941, 3, 4)
			GiveItemX(Player, 0, 9612, 3, 4)
		end
		AddMoney(Player, 0, Money)
		ScrollNotice("[Chaos Icicle]: Congratulations "..GetChaDefaultName(Player)..", you are the winner of Chaos Icicle on this round!",1, 0XFFF6E58D)
	return	
	end
end
-- at end of ctrl.lua
function ChaoisIciGetReaperPlayer()
    local chaosicicle_kills = PANEL_PLAYER_KILL_COUNT["chaosicicle"]
    if not chaosicicle_kills then
        return "N/A", 0
    end
    ...
    for player_name, kills in pairs(chaosicicle_kills) do
...
function ChaosIciUpdateKdrPanel(character)
    local map_copy = GetChaMapCopy(character)
    local character_name = GetChaDefaultName(character)

    -- Ensure the "chaosicicle" map entry exists in PANEL_PLAYER_KILL_COUNT
	PANEL_PLAYER_KILL_COUNT["chaosicicle"] = PANEL_PLAYER_KILL_COUNT["chaosicicle"] or {}
	PANEL_PLAYER_DEATH_COUNT["chaosicicle"] = PANEL_PLAYER_DEATH_COUNT["chaosicicle"] or {}
	PANEL_LAST_PLAYER_KILLED["chaosicicle"] = PANEL_LAST_PLAYER_KILLED["chaosicicle"] or {}

    -- Now safely access the character kills, deaths and last player killed
    local character_kills = PANEL_PLAYER_KILL_COUNT["chaosicicle"][character_name] or 0
    local character_deaths = PANEL_PLAYER_DEATH_COUNT["chaosicicle"][character_name] or 0
    local last_player_killed = GetChaDefaultName(PANEL_LAST_PLAYER_KILLED["chaosicicle"][character_name]) or "N/A"
	local chaos_reaper_player_name, chaos_reaper_kills = ChaoisIciGetReaperPlayer()
	local survivor = GetMapActivePlayer(map_copy)
...
-- chaosicicleNotice = function(map_copy)
	-- local Message = '[Chaos Icicle]: Total Players Alive: '..GetMapActivePlayer(map_copy)
	-- if table.getn(chaosicicle_GuildLog) ~= 0 then
		-- for GuildID, GuildCount in pairs(chaosicicle_GuildLog) do
		-- if GuildID == 0 then
