...
function get_map_entry_pos_springpk()
	return 282,373
end
...
function Springpk_Init()
	PANEL_PLAYER_KILL_COUNT["springpk"] = {}
    PANEL_PLAYER_DEATH_COUNT["springpk"] = {}
	PANEL_LAST_PLAYER_KILLED["springpk"] = {}
	Springpk_Active_Player = 0
	Springpk_Money = 1000000
	Springpk_Wave1 = 0
	Springpk_Wave2 = 0
	Springpk_Wave3 = 0
	Springpk_Wave4 = 0
...
function after_enter_springpk( Player , map_copy )
	Restriction.Add(Player, 'springpk', 'MAC')
	-- local GuildID = GetChaGuildID(Player)
	-- if GuildID ~= 0 then
		-- Springpk_GuildLog[GuildID] = Springpk_GuildLog[GuildID] or 0
		-- Springpk_GuildLog[GuildID] = Springpk_GuildLog[GuildID] + 1
	-- else 
		-- Springpk_GuildLog[0] = Springpk_GuildLog[0] or 0
		-- Springpk_GuildLog[0] = Springpk_GuildLog[0] + 1
	-- end
end
function before_leave_springpk( Player )
	Notice(GetChaDefaultName(Player)..' has left Chaos Spring!')
	Restriction.Rem(Player, 'springpk', 'MAC')
	...
	-- if GuildID ~= 0 then
		...
		-- if Springpk_GuildLog[GuildID] <= 0 then
			-- Springpk_GuildLog[GuildID] = nil
		-- end
		-- else 
		-- Springpk_GuildLog[0] = Springpk_GuildLog[0] or 0
		-- Springpk_GuildLog[0] = Springpk_GuildLog[0] - 1
		
		-- if Springpk_GuildLog[0] <= 0 then
			-- Springpk_GuildLog[0] = nil
		-- end
	-- end
end
function map_copy_first_run_springpk( map_copy )
	
end
function map_copy_run_springpk( map_copy )
local Survivor = GetMapActivePlayer(map_copy)
	local MapName = "Chaos Spring"
	SetMapEntryEventName(PortalEntryData7.springpk, ""..MapName.." | [ "..Survivor.." ] Survivor(s)")
	local Hour = tonumber(os.date("%H"))
	local Minute = tonumber(os.date("%M"))
	local Second = tonumber(os.date("%S"))
	local Message = "Chaos Spring still have survivor(s): "..Survivor.." player(s)!"
	DealAllPlayerInMap(map_copy,"SpringpkUpdateKdrPanel")
	if Hour == 4 or Hour == 16 then
		if Minute == 5 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave1 == 0 then
				local Monster2	= CreateChaEx(859, 6352, 3558, 145, 300, map_copy)
				local Monster3	= CreateChaEx(859, 6761, 13062, 145, 300, map_copy)
				local Monster4	= CreateChaEx(859, 10820, 24989, 145, 300, map_copy)
				local Monster5	= CreateChaEx(859, 14673, 20803, 145, 300, map_copy)
				local Monster6	= CreateChaEx(859, 12311, 16188, 145, 300, map_copy)
				local Monster7	= CreateChaEx(859, 12015, 13994, 145, 300, map_copy)
				local Monster8	= CreateChaEx(859, 12380, 12324, 145, 300, map_copy)
				local Monster9	= CreateChaEx(859, 14339, 12947, 145, 300, map_copy)
				local Monster10	= CreateChaEx(859, 18442, 12863, 145, 300, map_copy)
				local Monster11	= CreateChaEx(859, 18791, 7534, 145, 300, map_copy)
				local Monster12	= CreateChaEx(859, 14373, 7541, 145, 300, map_copy)
				local Monster13 = CreateChaEx(859, 10186, 6273, 145, 300, map_copy)
				local Monster14 = CreateChaEx(859, 10043, 9649, 145, 300, map_copy)
				local Monster15 = CreateChaEx(859, 17186, 3343, 145, 300, map_copy)
				...
				--GMNotice(Message)
				Springpk_Wave1 = Springpk_Wave1 + 1
			end
		end
		if Minute == 10 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave2 == 0 then
				local Monster16 = CreateChaEx(860, 11339, 24276, 145, 300, map_copy)
				local Monster17 = CreateChaEx(860, 13812, 18439, 145, 300, map_copy)
				local Monster18 = CreateChaEx(860, 15189, 6218, 145, 300, map_copy)
				local Monster19 = CreateChaEx(860, 16393, 12239, 145, 300, map_copy)
				local Monster20 = CreateChaEx(860, 14895, 13905, 145, 300, map_copy)
				local Monster21 = CreateChaEx(860, 12824, 12767, 145, 300, map_copy)
				local Monster22 = CreateChaEx(860, 10154, 11890, 145, 300, map_copy)
				...
				--GMNotice(Message)
				Springpk_Wave2 = Springpk_Wave2 + 1
			end
		end
		if Minute == 15 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave3 == 0 then
				local Monster23 = CreateChaEx(861, 12167, 25665, 145, 300, map_copy)
				local Monster24 = CreateChaEx(861, 13564, 23189, 145, 300, map_copy)
				local Monster25 = CreateChaEx(861, 18274, 13329, 145, 300, map_copy)
				local Monster26 = CreateChaEx(861, 19167, 9778, 145, 300, map_copy)
				local Monster27 = CreateChaEx(861, 14915, 14352, 145, 300, map_copy)
				local Monster28 = CreateChaEx(861, 13881, 9818, 145, 300, map_copy)
				local Monster29 = CreateChaEx(861, 10854, 11740, 145, 300, map_copy)
				...
				--GMNotice(Message)
				Springpk_Wave3 = Springpk_Wave3 + 1
			end
		end
		if Minute == 20 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave4 == 0 then
				...
				--GMNotice(Message)
				Springpk_Wave4 = Springpk_Wave4 + 1
			end
		end
		if Minute == 25 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave5 == 0 then
				...
				GMNotice(Message)
				Springpk_Wave5 = Springpk_Wave5 + 1
			end
		end
		if Minute == 30 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave6 == 0 then
				...
				--GMNotice(Message)
				Springpk_Wave6 = Springpk_Wave6 + 1
			end
		end
		if Minute == 35 and Second == 0 then
		GMNotice(Message)
			if Springpk_Wave7 == 0 then
				...
				local Monster334 = CreateChaEx(1839, 12850, 14175, 205, 60, map_copy)
				...
				--GMNotice(Message)
				Springpk_Wave7 = Springpk_Wave7 + 1
			end
		end
		if Minute >= 40 and Minute < 45 then
			if Springpk_Tick == 300 then
				Notice("Chaos springpk will be closed in 5 minutes - "..Survivor.." Survivor(s) !")
			end
			if Springpk_Tick == 180 then
				Notice("Chaos springpk will be closed in 3 minutes - "..Survivor.." Survivor(s) !")
			end
			if Springpk_Tick == 120 then
				Notice("Chaos springpk will be closed in 2 minutes - "..Survivor.." Survivor(s) !")
			end
			if Springpk_Tick == 60 then
				Notice("Chaos springpk will be closed in 60 seconds - "..Survivor.." Survivor(s) !")
			end
			Springpk_Tick = Springpk_Tick - 1
		end
	end
end
function map_copy_run_special_springpk( map_copy )

end

function map_run_springpk( map )

end
function map_copy_close_springpk( map_copy )
	ClearAllSubMapCha(map_copy)
	ClearAllSubMapMonster(map_copy)
end
function map_copy_before_close_springpk( map_copy )
	PANEL_PLAYER_KILL_COUNT["springpk"] = {}
    PANEL_PLAYER_DEATH_COUNT["springpk"] = {}
	Springpk_Active_Player = GetMapActivePlayer(map_copy)
	if Springpk_Active_Player > 0 then
		DealAllActivePlayerInMap(map_copy, "ChaosspringpkRewards")
	end
	Springpk_Init()
end

function can_open_entry_springpk( map )
...
function ChaosspringpkRewards(Player)
	if Springpk_Active_Player == 0 then
		return
	end
	local Money = math.floor(Springpk_Money/Springpk_Active_Player)
	if Springpk_Active_Player > 5 then
		AddMoney(Player, 0, Money)
		return
	end
	if Springpk_Active_Player <  5 and Springpk_Active_Player > 1 then
	if GetChaFreeBagGridNum(Player) >= 4 then
	...
	----------------------------------
	if Springpk_Active_Player == 1 then
		if GetChaFreeBagGridNum(Player) >= 6 then
			GiveItem(Player, 0, 3878, 1, 4)
			GiveItem(Player, 0, 9941, 5, 4)
			GiveItem(Player, 0, 9612, 3, 4)		
			GiveItem(Player, 0, 9939, 2, 4)	
			GiveItem(Player, 0, 9989, 1, 4)	
		else
			GiveItemX(Player, 0, 3878, 1, 4)
			GiveItemX(Player, 0, 9941, 5, 4)
			GiveItemX(Player, 0, 9612, 3, 4)
			GiveItemX(Player, 0, 9939, 2, 4)	
			GiveItemX(Player, 0, 9989, 1, 4)
		end
		AddMoney(Player, 0, Money)
		ScrollNotice("[Chaos Spring]: Congratulations "..GetChaDefaultName(Player)..", you are the winner of Chaos Spring on this round!",1, 0XFFF6E58D)
	return	
	end
end
function SpringpkGetReaperPlayer()
    local springpk_kills = PANEL_PLAYER_KILL_COUNT["springpk"]
    if not springpk_kills then
        return "N/A", 0
    end
    ...
    for player_name, kills in pairs(springpk_kills) do
...
---- KDR PANEL
function SpringpkUpdateKdrPanel(character)
    local map_copy = GetChaMapCopy(character)
    --local survivor = GetMapActivePlayer(map_copy)
    local character_name = GetChaDefaultName(character)

    -- Ensure the "springpk" map entry exists in PANEL_PLAYER_KILL_COUNT
	PANEL_PLAYER_KILL_COUNT["springpk"] = PANEL_PLAYER_KILL_COUNT["springpk"] or {}
	PANEL_PLAYER_DEATH_COUNT["springpk"] = PANEL_PLAYER_DEATH_COUNT["springpk"] or {}
	PANEL_LAST_PLAYER_KILLED["springpk"] = PANEL_LAST_PLAYER_KILLED["springpk"] or {}

    -- Now safely access the character kills, deaths and last player killed
    local character_kills = PANEL_PLAYER_KILL_COUNT["springpk"][character_name] or 0
    local character_deaths = PANEL_PLAYER_DEATH_COUNT["springpk"][character_name] or 0
    local last_player_killed = GetChaDefaultName(PANEL_LAST_PLAYER_KILLED["springpk"][character_name]) or "N/A"
	local chaos_reaper_player_name, chaos_reaper_kills = SpringpkGetReaperPlayer()
	local survivor = GetMapActivePlayer(map_copy)
...
-- SpringpkNotice = function(map_copy)
	-- local Message = '[Chaos Spring]: Total Players Alive: '..GetMapActivePlayer(map_copy)
	-- if table.getn(Springpk_GuildLog) ~= 0 then
		-- for GuildID, GuildCount in pairs(Springpk_GuildLog) do
		-- if GuildID == 0 then
